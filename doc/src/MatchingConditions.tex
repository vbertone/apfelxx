...
contributions are absent.\footnote{In fact, the discussion also
  assumed that the PDF matching takes place at the heavy quark-mass
...
and that they are identically vanishing below threshold. While this
turns out to be a good approximation for PDFs this is not the case for
...
thresholds. Importantly, the perturbative expansion of these
additional functions is different from zero at $\mathcal{O}(\alpha_s)$
even choosing $\mu_h=m_h$, with $\mu_h$ the matching scale, producing
a discontinuity already at NLO.  However, in what follows we will
assume that matching conditions are not computed beyond
$\mathcal{O}(\alpha_s^2)$, \textit{i.e.}  NNLO and that intrinsic
heavy-quark and heavy-antiquark contributions are equal, \textit{i.e.}
$h=\overline{h}$ below threshold. This will allow is to concentrate
the discussion on the singlet sector because the non-singlet one
remains diagonal. Specifically, these assumptions lead to:
\begin{equation}
...
V_i^{(N_f+1)}(\mu_h) = [1+K_{ll}](\mu_h) V^{(N_f)}(\mu_h) = [1+K^{\rm
  NS}(\mu_h)] V_i^{(N_f)}(\mu_h)\,,\quad i = 3,8,15,24,35\,,
\end{equation}
with $K_{ll}$ some given function that starts at
$\mathcal{O}(\alpha_s^2)$. This equivalently amounts to say that no
quark-antiquark asymmetry is generated by the matching procedure.

We now focus on the singlet sector and generalise
Eqs.~(\ref{eq:lightqmc}), (\ref{gluon}), and~(\ref{eq:heavyqmc})
(changing and simplifying a bit the notation) as:
...
remind that $h$ is the $(N_f+1)$-th heaviest flavour, \textit{i.e.}
...
\begin{equation}
P^{(N_f+1)} = \left[\mathbb{I}+\mathbb{K}^{(N_f)}\right]P^{(N_f+1)}\,,
\end{equation}
...
Now one may want to rotate the matching matrices from the physical
basis to the evolution basis through $T\mathbb{K}^{(N_f)}T^{-1}$,
where $T$ transforms the vector $P$ into the evolution-basis vector
$E=(g,\Sigma,T_3,T_8,T_{15},T_{24},T_{35})$. However, it turns out the
resulting matching matrix are much more complicated and generally less
sparse than those in Eq.~(\ref{eq:MathcingMatrices}). Therefore, it
seems convenient to perform the matching in the physical basis.
