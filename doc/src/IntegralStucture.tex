...
\end{equation}
While the limit $d$ is manifestly depending only on the difference
...
at $x_{N_x}=1$ and thus the symmetry is effectively preserved. The
consequence of this symmetry is that computing, \textit{e.g.}, the
integrals $a_\alpha=I_{0\alpha}$ for $\alpha=0,\dots,N_x$ is enough to
reconstruct the full set of $I_{\beta\alpha}$ because, in matricial
representation, it will look like this:
...
I_{\beta\alpha} = \sum_{j={\rm max}[0, \alpha + 1 - N_x]}^{{\rm min}[\kappa, \alpha - \beta]}  \int_{x_\beta/x_{\beta-j+1}}^{x_\beta/x_{\beta-j}} dy\,O(y)w_{\alpha}\left(\frac{x_\beta}{y}\right)\,,
\end{equation}
...
\begin{equation}
I_{\beta\alpha} = \sum_{j={\rm max}[0, \alpha + 1 - N_x]}^{{\rm min}[\kappa, \alpha - \beta]}  \int_{s^{j-1}}^{s^{j}} dy\,O(y)w_{\alpha}\left(\frac{x_\beta}{y}\right)\,.
\end{equation}
...
J_{\beta\alpha} = \sum_{j={\rm max}[0, \alpha + 1 - N_x]}^{\kappa}  \int_{x_\beta/x_{\beta-j+1}}^{x_\beta/x_{\beta-j}} dy\,O(y)w_{\alpha}\left(\frac{x_\beta}{y}\right)=\sum_{j={\rm max}[0, \alpha + 1 - N_x]}^{\kappa}  \int_{s^{j-1}}^{s^{j}} dy\,O(y)w_{\alpha}\left(\frac{x_\beta}{y}\right)\,.
\end{equation}
Therefore, the only difference with respect to the ``standard'' case
is a change of the upper summation bound. It should be clear, however,
that this procedure necessarily implies an approximation. This is due
to the fact that the interpolation grid, understood to be
logarithmically distributed, does not reach zero. This approximation
manifests itself in the fact that upper bound of the intergrals in
Eq.~(\ref{eq:ERBLints}) never reaches infinity but it gets as large as
$s^{\kappa}$. The size of $s^{\kappa}$ can range widely depending on
the lower bound of the grid $x_0$, on the number of nodes $N_x$, and
on the interpolation degree $\kappa$, according to:
\begin{equation}
...
% It is instructive to generalise the above procedure by considering
% integrals of this kind:
% \begin{equation}\label{eq:ZMconvERBLGen}
%   K(x) = x\int_{b(x)}^1\frac{dz}{z} O \left(\frac{x}{z}\right) d(z)\,,
% \end{equation}
% where $b$ can be any function of the external parameter $x$. Following
% the same steps taken above, one arrives at:
% \begin{equation}
%   K(x_\beta)= \sum_{\alpha=0}^{N_x} K_{\beta\alpha}\overline{d}_\alpha\,,
% \end{equation}
% where:
% \begin{equation}
% K_{\beta\alpha} = \sum_{j={\rm max}[0, \alpha + 1 - N_x]}^{???}  \int_{s^{j-1}}^{s^{j}} dy\,O(y)w_{\alpha}\left(\frac{x_\beta}{y}\right)\,.
% \end{equation}
