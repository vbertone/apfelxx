...
\begin{array}{rclclcl}
  ...
  &+&
     {\rm SS}\,\Gamma_{\beta\delta}^{\rm
      S0}\Gamma_{\alpha\gamma}^{\rm S0}&+& \Gamma_{\beta\delta}^{\rm
      S0}\Gamma_{\alpha\gamma}^{\rm SR}\\
\\
&+& \displaystyle \left[\Gamma_{\beta\delta}^{\rm RL}+\ln(1-c_{\alpha\gamma})\Gamma_{\beta\delta}^{\rm
                                    RS}\right]\delta_{\alpha\gamma} &+& \Gamma_{\beta\delta}^{\rm RS}\Gamma_{\alpha\gamma}^{\rm
...
Despite writing the hard cross sections as in
Eq.~(\ref{eq:DoubleFuncStruct}) requires a bit of analytical work, it
is extremely beneficial from the numerical point of view because it
allows us to reduce complicated expressions of two variables to a
combination of products of functions of one single variable that can
be treated as discussed in the previous sections. In particular, the
pre-computation is limited to the integrals in
Eq.~(\ref{eq:PrecompOp}). We stress however that the structure in
Eq.~(\ref{eq:DoubleFuncStruct}) is valid only for the NLO corrections
to SIDIS and (possibly) DY. We expect that, when going beyond NLO, the
structure of the hard cross sections would get more complicated
essentially for the presence of more involved plus-prescripted
functions. However, we also expect that the same kind of
``factorisation'' can be implemented.

We now apply this technology to the specific cases of SIDIS first and
DY second to NLO in QCD.
