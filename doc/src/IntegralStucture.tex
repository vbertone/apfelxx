...
\begin{equation}\label{eq:FinalExpression}
\Gamma_{\beta\alpha} = \int_{c}^{d} dz\left\{R\left(z\right)w_{\alpha}^{(k)}\left(\frac{x_\beta}{z}\right)+\frac{S^{(0)}}{1-z}\left[w_{\alpha}^{(k)}\left(\frac{x_\beta}{z}\right)-\delta_{\alpha\beta}\right]\right\}+\displaystyle \left[S^{(0)}\ln\left(1-c\right)+L \right]\delta_{\alpha\beta}\,.
\end{equation}
