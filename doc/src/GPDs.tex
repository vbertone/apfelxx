...
\displaystyle \mathcal{P}_1^{-,(0)}(y,\kappa) &=& \displaystyle
                                                  2P_{\rm NS}(y,\kappa y) + \delta(1-x)2C_F\left(\frac{3}{2}-2\int_{0}^{1}\frac{dy}{1-y}-\ln(|1-\kappa^2|)\right) \\
\\
...
\displaystyle \mathcal{P}_1^{-,(0)}(y,\kappa)=2P_{\rm NS}'(y,\kappa y) +2P_{\rm NS}'(y,-\kappa y)+ \delta(1-x)2C_F\left(\frac{3}{2}-2\int_{0}^{1}\frac{dy}{1-y}-\ln(|1-\kappa^2|)\right)\,,
\end{equation}
with $P_{\rm NS}'$ is given in Eq.~(19) of Ref.~\cite{Ji:1996nm} and
remarkably equal to the expression in Eq.~(\ref{eq:p1minus0})
signifying that:
\begin{equation}
  P_{\rm NS}(y,\kappa y) =  P_{\rm NS}'(y,\kappa y) +P_{\rm NS}'(y,-\kappa y)\,.
\end{equation}
While:
\begin{equation}\label{eq:p2minus0}
  \mathcal{P}_2^{-,(0)}(y,\kappa) = -2P_{\rm NS}'(y,-\kappa y) +2P_{\rm
    NS}'(-y,\kappa y) = 2C_F(\kappa-1)\frac{y+(1+2\kappa)y^3}{(1-y^2)(1-\kappa^2y^2)}\,.
\end{equation}
It is very interesting to notice that $\mathcal{P}_2^{-,(0)}$ is
proportional to $(\kappa-1)$ that guarantees the continuity of GPDs at
$k=1$.
...
\begin{equation}
\begin{array}{rcl}
V_{\rm NS}^{(0)}(x,y) &=&\displaystyle \theta(y-x)\left[\frac{1}{y}P_{\rm
    NS}'\left(\frac{x}{y},\frac{1}{x}\right)
                          \right]-2C_F\delta\left(y-x\right)\int_{-1}^1 dz \,\frac{\theta(z-x)}{z-x}\\
\\
&+&\displaystyle \theta(x-y)\left[-\frac{1}{y}P_{\rm
    NS}'\left(\frac{x}{y},-\frac{1}{x}\right)
...
where:
\begin{equation}
\frac{1}{y}P_{\rm NS}'\left(\frac{x}{y},\frac{1}{x}\right)=2C_F\frac{1+x}{1+y}\left(\frac{1}{2}+\frac{1}{y-x}\right)\,.
\end{equation}
...
\\
&-&\displaystyle \theta(t-u)\left(\frac{t}{1-u}+\frac1{u-t}-\delta(t-u)\int_0^1 du'\frac{\theta(t-u')}{u'-t}\right) +\frac34\delta\left(u-t\right)\bigg]\,.
\end{array}
\end{equation}
Now, with a slight abuse of notation, we define:\footnote{The abuse of
  notation stems from the fact that this definition of the
  $+$-prescription is not identical to the usual one. The presence of
  the $\theta$-functions is crucial to avoid a principal value and
  guarantee the cancellation of the divergence proper to the
  $+$-prescription.}
\begin{equation}
\theta\left[\pm\left(u-t\right)\right]f(t,u)-\delta(u-t)\int_0^1du'\,\theta\left[\pm\left(u'-t\right)\right]f(t,u')\equiv \left[f(t,u)\right]_+\,,
\end{equation}
...
\begin{equation}\label{eq:anomdim1}
  ...
  2C_F\left\{\theta(u-t)\left[\frac{t-1}{u}+\left(\frac1{u-t}\right)_+\right]-\theta(t-u)\left[\frac{t}{1-u}+\left(\frac1{u-t}\right)_+\right]+\frac32\delta\left(u-t\right)\right\}\,.
\end{equation}
This confirms the result of Ref.~\cite{Blumlein:1999sc}.
...
from Ref.~(\cite{Ji:1996nm}) admits a fully $+$-prescribed form. This
was also explicitly derived in Ref.~\cite{Mikhailov:1984ii} and argued
that this property must for symmetry reasons.
...
$+$-prescribed form violate any conservation law? To answer this question,
we notice that the fact that non-singlet DGLAP anomalous dimension
integrates to zero (see Eq.~(\ref{eq:dglapsplitting})), and thus
admits a $+$-prescribed form, derives from the conservation of the
total number of quarks minus anti-quarks (valence sum rule):
\begin{equation}
...
Eq.~(\ref{eq:jitot}). In doing this one needs to take into account
that $\kappa=\xi/x$:
\begin{equation}
\begin{array}{rcl}
...
\end{equation}
such that (to my big relief) Eq.~(\ref{eq:valsumruleGPD}) is
fulfilled. Despite Eq.~(\ref{eq:valsumruleGPD}) has been explicitly
proved at one-loop, the same relation must hold order by order in
perturbation theory.
...
reproduces the one-loop DGLAP splitting functions.
