...
\displaystyle \frac{d H_{\rm NS}(x,0,Q^2)}{d\ln
  Q^2}&=&\displaystyle\frac{2\alpha_s(Q^2)}{3\pi}\Bigg[\int_x^1 dy
          \frac{x^2+y^2}{y (y-x)}\frac{1}{y}\left(H_{\rm
...
\begin{array}{rcl}
  \displaystyle \frac{d H_{\rm NS}(x,0,Q^2)}{d\ln
    Q^2}&=&\displaystyle \frac{2\alpha_s(Q^2)}{3\pi}\Bigg[\int_x^1
            dy\,\frac{x^2+y^2}{y(y-x)}\left(\frac1y
            H_{\rm NS}\left(y,0,Q^2\right)-H_{\rm
      NS}(x,0,Q^2)\right)\\
\\
 &+&\displaystyle H_{\rm NS}(x,0,Q^2)\left(\frac{3}{2}+2\ln(1-x)\right)\Bigg]\,,
\end{array}
\end{equation}
which differs from Eq.~(\ref{eq:Vinnikov}) by the fact that the factor
$1/y$ does not multiply the subtraction term. In fact, the difference
between Eqs.~(\ref{eq:Vinnikov}) and~(\ref{eq:DGLAPexpl}) can be
(partially) traced back to the discrepancy discussed in
Sect.~(\ref{sec:endpoint}).
