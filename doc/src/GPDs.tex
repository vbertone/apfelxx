...
\displaystyle\mu^2\frac{d}{d\mu^2}f(\pm x,\xi) &=&\displaystyle
  \int_{|x|}^{1}\frac{dx'}{x'}\left[\frac{x'}{\left|2\xi\right|}P\left(\pm \frac{x}{\xi},\frac{x'}{\xi}\right)f(x',\xi)+\frac{x'}{\left|2\xi\right|}P\left(\mp \frac{x}{\xi},\frac{x'}{\xi}\right)f(-x',\xi)\right]\\
\\
&+&\displaystyle\theta\left(1-\left|\frac{x}{\xi}\right|\right)
  \int_{0}^{|x|}dx'\left[\frac{1}{\left|2\xi\right|}P\left(\pm \frac{x}{\xi},\frac{x'}{\xi}\right)f(x',\xi)+\frac{1}{\left|2\xi\right|}P\left(\mp \frac{x}{\xi},\frac{x'}{\xi}\right)f(-x',\xi)\right]\,.
\end{array}
\end{equation}
...
\displaystyle\mu^2\frac{d}{d\mu^2}f^{\pm}(x,\xi) &=&
  \displaystyle\int_{|x|}^{1}\frac{dx'}{x'}\frac{x'}{\left|2\xi\right|}
                                                         P^{\pm}\left(\frac{x}{\xi},\frac{x'}{\xi}\right)f^{\pm}(x',\xi)+\theta\left(1-\left|\frac{x}{\xi}\right|\right)
  \int_{0}^{|x|}dx'\frac{1}{\left|2\xi\right|}P^{\pm}\left(\frac{x}{\xi},\frac{x'}{\xi}\right)f^{\pm}(x',\xi)\\
\\
...
regions. As indicated by the subscripts, $I^{\pm,\rm DGLAP}$ and
$I^{\pm,\rm ERBL}$ define the so-called DGLAP and ERBL regions in $x$
relative $\xi$. Specifically, the presence of the $\theta$-function in
$I^{\pm,\rm ERBL}$ is such that for $x>\xi$ this term drops leaving
only the DGLAP-like term $I^{\pm,\rm DGLAP}$. For $x\leq\xi$, instead,
...
\begin{equation}
  I^{\pm,\rm DGLAP}(\xi,x)=\int_{|x|}^{1}\frac{dx'}{x'}\frac{x'}{\left|2\xi\right|}
                                                         P^{\pm}\left(\frac{x}{\xi},\frac{x'}{\xi}\right)f^{\pm}(x',\xi)=
                                                         \int_{|x|}^{1}\frac{dx'}{x'}\mathcal{P}^{\pm}\left(\kappa,\frac{x}{x'}\right)f^{\pm}(x',\xi)\equiv
...
\begin{equation}
x_\beta I^{\pm,\rm DGLAP}(\xi, x_\beta) = \sum_\alpha \mathcal{P}_{\beta\alpha}^{\pm,\rm DGLAP}(\xi) f^\pm_\alpha (\xi)\,,
\end{equation}
with:
\begin{equation}
...
\mathcal{P}_{\beta\alpha}^{\pm, \rm
  DGLAP}(\xi) = \int_{c}^{d}dx'\,\mathcal{P}^\pm\left(\kappa_\beta,x'\right)w_\alpha^{(k)}\left(\frac{x_\beta}{x'}\right)\,,
\end{equation}
...
\begin{equation}
\mathcal{P}_{\beta\alpha}^{\pm, \rm
  ERBL}(\xi)=\theta\left(1-\frac{1}{|\kappa_\beta|}\right) x_\beta\int_{\epsilon}^{x_\beta}dx'\,\mathcal{P}^\pm\left(\kappa_\beta,\frac{x_\beta}{x'}\right) \frac{1}{x'^2} w_\alpha^{(k)}(x')\,,
\end{equation}
...
\begin{equation}
  \mathcal{P}_{\beta\alpha}^{\pm,\rm
    ERBL}(\xi)=\theta\left(1-\frac{1}{|\kappa_\beta|}\right) x_\beta\int_{a}^{b}dx'\,\mathcal{P}^\pm\left(\kappa_\beta,\frac{x_\beta}{x'}\right) \frac{1}{x'^2} w_\alpha^{(k)}(x')\,,
\end{equation}
with:
\begin{equation}
...
\begin{equation}
I(x)=\int_x^1dy\,\left[\frac{F(y)-F(1)}{1-y}+F(1)\left(\frac{1}{1-y}\right)_++\left(L+S\ln(1-x)\right)\delta(1-y)\right]f(y)\,.
\end{equation}
...
At leading-order (LO), we find:
\begin{equation}
\mathcal{P}_{qq,\rm NS}^{\pm,[0]} (\kappa,y) = 2C_F\left[-\frac{1+y}{1-\kappa^2 y^2}+2\left(\frac{1}{1-y}\right)_+-\left(\frac{(1+\kappa)\ln(1-\kappa)+(1-\kappa)\ln(1+\kappa)}{2\kappa^2}\right)\delta(1-x)\right]\,,
\end{equation}

\begin{equation}
\mathcal{P}_{qg}^{\pm,[0]} (\kappa,y) = 4n_f T_R \frac{1-y(2-(2-\kappa^2)y)}{(1-\kappa^2 y^2)^2}\,,
\end{equation}

\begin{equation}
\mathcal{P}_{gq}^{\pm,[0]} (\kappa,y) = 2C_F\frac{2-y(2-(1-\kappa^2)y)}{y(1-\kappa^2 y^2)}\,,
\end{equation}

\begin{equation}
\mathcal{P}_{gg}^{\pm,[0]} (\kappa,y) = 
\end{equation}
