...
\displaystyle\mu^2\frac{d}{d\mu^2}f^{\pm}(x,\xi)= {\rm sign}(\xi)\int_{x}^{x/b(x)}\frac{dy}{y}\mathcal{P}^{\pm}\left(\kappa,y\right)f^{\pm}\left(\frac{x}{y},\xi\right)\,,
\end{equation}
with:
\begin{equation}
...
\subsection{A note on continuity of GPDs}
...
and~(\ref{eq:limit2}), one finds, at least at one loop, that:
\begin{equation}
...
\end{equation}
This appears to be some sort of sum rule that the GPDs and/or
anomalous dimensions in the ERBL region have to fullfil.
