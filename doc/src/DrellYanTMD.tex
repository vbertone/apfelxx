...
\overline{p}_T(\cos\phi) = \frac{Q^2}{2 \left[M\cosh\left(\eta - y\right)-|\mathbf{q}_T|\cos\phi\right]}\,.
\end{equation}
...
\end{equation}
We now use Eqs.~(\ref{eq:intoverphi}) and~(\ref{eq:defintoverphi}) to
compute:
\begin{equation}
\frac{1}{Q^2}\int_0^{2\pi}d\phi\,[\overline{p}_T(\cos\phi)]^2 = \frac{2\pi
  Q^2M\cosh\left(\eta -
...
\begin{array}{l}
  ...
  d\eta\frac{2\pi Q^2M\cosh(\eta-y)}{(M^2\cosh^2(\eta-y)-|\mathbf{q}_T|^2)^{3/2}}= \\
  \\
  \displaystyle\pi \frac{Q^2}{M^2}\int_{-\infty}^\infty
  \frac{d(\sinh\eta)}{\left(\sinh^2(\eta-y)+\frac{Q^2}{M^2}\right)^{3/2}}= 2\pi \frac{Q^2}{M^2}\left[\frac{M^2}{Q^2}\frac{\sinh\eta}{\sqrt{\sinh^2\eta+\frac{Q^2}{M^2}}}\right]_{-\infty}^{\infty}= 4\pi\,.
\end{array}
\end{equation}
% \newpage
% To so, it is convenient to express the four-vector $q$ in terms of
% $Q$, $y$, and $\mathbf{q}_T$:
% \begin{equation}
% q=\left(M\cosh y,\mathbf{q}_T,M\sinh y\right)\,.
% \end{equation}
...
% \begin{equation}\label{eq:pTpm}
% p_T^{-} \leq |\mathbf{p}_T| \leq p_T^{+}\quad\mbox{with}\quad
% p_T^{\pm}=\frac{Q^2}{M\cosh(\eta-y)\mp |\mathbf{q}_T|}\,.
% \end{equation}
% In principle, one should also take into account the $\theta$-function
% in Eq.~(\ref{eq:integralyeah!}) that guarantees the positiveness of
% the energy:
% \begin{equation}
% \theta(q_0-p_0)=\theta(M\cosh y-|\mathbf{p}_T|\cosh\eta)\quad\Rightarrow\quad |\mathbf{p}_T|\leq
% p_T^{(E)} = \frac{M\cosh y}{\cosh\eta}\,.
% \end{equation}
% However, it turns out that this constraint is always fulfilled within
% the interval defined in Eq.~(\ref{eq:pTpm}) (To be
% verified!). Finally, we have:
% \begin{equation}\label{eq:PSintegral}
% \begin{array}{l}
% \displaystyle \int d^4p \delta(p^2) \delta((q-p)^2) \theta(p_{0})
%     \theta(q_0-p_{0})=\\
% \\
%  \displaystyle \int_{-\infty}^\infty d\eta
% \int_{p_T^{-}}^{p_T^{+}}
% \frac{d|\mathbf{p}_T||\mathbf{p}_T|}{\sqrt{4|\mathbf{p}_T|^2\left[|\mathbf{q}_T|^2-M^2\cosh^2\left(\eta
%   - y\right)\right] +4|\mathbf{p}_T|Q^2 M\cosh\left(\eta - y\right)
%   -Q^4}}=\\
% \\
% \displaystyle \frac1{Q^2}\int_{-\infty}^\infty d\eta \sqrt{p_T^+p_T^-}
% \int_{p_T^{-}}^{p_T^{+}}
% \frac{d|\mathbf{p}_T||\mathbf{p}_T|}{\sqrt{(|\mathbf{p}_T|-p_T^-)(p_T^+-|\mathbf{p}_T|)}}\,.
% \end{array}
% \end{equation}
% In turns out that the primitive function of the integrand in
% $|\mathbf{p}_T|$ can be computed analytically:
% \begin{equation}
% \int
% \frac{d|\mathbf{p}_T||\mathbf{p}_T|}{\sqrt{(|\mathbf{p}_T|-p_T^-)(p_T^+-|\mathbf{p}_T|)}}
% = \sqrt{(|\mathbf{p}_T|-p_T^-)(p_T^+-|\mathbf{p}_T|)}+\frac{p_T^-
%   +p_T^+}2\tan^{-1}\left[\frac{p_T^- +p_T^+ - 2 |\mathbf{p}_T|}{2\sqrt{(|\mathbf{p}_T|-p_T^-)(p_T^+-|\mathbf{p}_T|)}}\right]\,,
% \end{equation}
% and is such that, if computed between $p_T^{-}$ and $p_T^{+}$, it
% gives:
% \begin{equation}
% \int_{p_T^-}^{p_T^+}
% \frac{d|\mathbf{p}_T||\mathbf{p}_T|}{\sqrt{(|\mathbf{p}_T|-p_T^-)(p_T^+-|\mathbf{p}_T|)}}
% = \frac{\pi(p_T^-+p_T^+)}2\,.
% \end{equation}
% We can go further and try to solve also the integral in $\eta$. To do
% so we need to use the explicit expressions for $p_T^{\pm}$ given in
% Eq.~(\ref{eq:pTpm}):
% \begin{equation}
% \begin{array}{l}
% \displaystyle \int d^4p \delta(p^2) \delta((q-p)^2) \theta(p_{0})
%     \theta(q_0-p_{0})=\frac{\pi}{2Q^2}\int_{-\infty}^\infty d\eta
%   (p_T^-+p_T^+)\sqrt{p_T^+p_T^-} =\\
% \\
% \displaystyle\pi Q^2\int_{-\infty}^\infty
%   d\eta\frac{M\cosh(\eta-y)}{(M^2\cosh^2(\eta-y)-|\mathbf{q}_T|^2)^{3/2}}
%   = \pi \frac{Q^2}{M^2}\int_{-\infty}^\infty
%   \frac{d(\sinh\eta)}{\left(\sinh^2(\eta-y)+\frac{Q^2}{M^2}\right)^{3/2}}=\\
% \\
% \displaystyle \pi \frac{Q^2}{M^2}\left[\frac{M^2}{Q^2}\frac{\sinh\eta}{\sqrt{\sinh^2\eta+\frac{Q^2}{M^2}}}\right]_{-\infty}^{\infty}= 2\pi\,.
% \end{array}
% \end{equation}
...
\\
\displaystyle \vartheta(\overline{p}_T - p_{T\rm min})
\vartheta(|\eta| - \eta_{\rm max}) \vartheta(\sqrt{|\mathbf{q}_T|^2+\overline{p}_T^2-2 |\mathbf{q}_T|\overline{p}_T\cos\phi} - p_{T\rm min})\\
\\
\displaystyle \times
  \vartheta\left(\eta_{\rm max}-\left|\frac12\ln\left(\frac{Me^y-\overline{p}_Te^\eta}{Me^{-y}-\overline{p}_Te^{-
\eta}}\right)\right|\right)\,,
\end{array}
\end{equation}
