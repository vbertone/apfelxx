...
\subsection{Derivation and integration of an interpolated function}
...
derivative of the test function $g$ in Eq.~(\ref{generalCase2}) only
knowing its values on the grid points as: 
\begin{equation}\label{DerGeneralCase2}
  \frac{dg}{dx} =
  \sum_{\beta=0}^{N_x+k-1}\frac{d\mathcal{I}_\beta^{(k)}}{dx} g(x_{\beta})\,,
\end{equation}
where:
\begin{equation}\label{eq:derintfunc}
  \frac{d\mathcal{I}_\beta^{(k)}}{dx} = \sum_{i=0\atop i\leq\beta}^k
...
the function $g$. To do so, we need to compute the integral:
\begin{equation}
L_i^{(k)}(a,b)=\int_a^bdy\,\ell_i^{(k)}(y) =\prod^{k}_{m=0\atop m\ne
i}\frac{1}{x_{\alpha+i}-x_{\alpha+m}}\int_a^b dy\prod^{k}_{m=0\atop m\ne
i}(y-x_{\alpha+m})\,.
\end{equation}
It is easy to see that:
\begin{equation}\label{eq:expansion}
\prod^{k}_{m=0\atop m\ne
i}(y-x_{\alpha+m})=\sum_{n=0}^k (-1)^n p_{\alpha+i}^{(k)}(n)y^{k-n}\,,
\end{equation}
such that:
\begin{equation}
L_i^{(k)}(a,b) =\left[\prod^{k}_{m=0\atop m\ne
i}\frac{1}{x_{\alpha+i}-x_{\alpha+m}}\right]\left[\sum_{n=0}^k\frac{(-1)^n p_{\alpha+i}^{(k)}(n)}{k-n+1}\left(b^{k-n+1}-a^{k-n+1}\right)\right]\,.
\end{equation}
What is left to do is to determine the coefficients
$p_{\alpha+i}^{(k)}$. For convenience, let us define the set
$\{r_1,\dots,r_k\}=\{x_\alpha,\dots,x_{\alpha+i-1},x_{\alpha+i+1},\dots,x_{\alpha+k}\}$.\footnote{From
  the definition of the set $\{r_1,\dots,r_k\}$, it becomes clear that
  the subscript $\alpha+i$ of the coefficients $p_{\alpha+i}^{(k)}$
  refers to the fact that this coefficients are computed on a vector
  of $k$ nodes where the node $x_{\alpha+i}$ has been removed.} The
coefficients $p_{\alpha+i}^{(k)}$ defined in Eq.~(\ref{eq:expansion})
can be expressed as:
\begin{equation}\label{eq:pncoef}
  p_{\alpha+i}^{(k)}(n)=\sum_{i_1=1}^k r_{i_1}\sum_{i_2=i_1+1}^k r_{i_2}\dots\sum_{i_n=i_{n-1}+1}^k r_{i_n}\,.
\end{equation}
In order to obtain a convenient algorithm to compute the coefficients
$p_{\alpha+i}^{(k)}$, we define the vectorial function:
\begin{equation}
\mathbf{f}^{(k)}(\mathbf{r},\mathbf{a})\quad\mbox{with components}\quad  f_j^{(k)}\left(\mathbf{r},\mathbf{a}\right)=\sum_{i=j+1}^k r_{i}a_{i-j}\,.
\end{equation}
It is important to notice that while $\mathbf{r}$ is a $k$-dimensional
vector with index running between 1 and $k$, $\mathbf{a}$ and are in
principle an infinite-dimensional vector with index running between
$-\infty$ and $+\infty$. However, given the definition of its
components in Eq.~(\ref{eq:pncoef}), it turns out that $f_j^{(k)}=0$
for $j\geq k$. The same applies to $\mathbf{a}$ because, as we will
see below, it has to be identified with some $\mathbf{f}^{(k)}$. The
function $\mathbf{f}^{(k)}$ can now be used to defined the vector
$\mathbf{P}^{(k)}(n)$ recursively. The relevant recursive relation is:
\begin{equation}
\mathbf{P}^{(k)}(n+1) = \mathbf{f}^{(k)}(\mathbf{r},\mathbf{P}^{(k)}(n))\quad\mbox{with}\quad\mathbf{P}^{(k)}(0) = \mathbf{1}\,.
\end{equation}
Finally, the coefficient $p_{\alpha+i}^{(k)}(n)$ is the zero-th
component of the vector $\mathbf{P}^{(k)}(n)$, \textit{i.e.}
$p_{\alpha+i}^{(k)}(n)\equiv P_0^{(k)}(n)$.

The integral of the function $g$ between $a$ and be can finally be
computed on the grid as:
\begin{equation}
\int_a^bdy\,g(y) = \sum_{\beta=0}^{N_x+k-1}\mathcal{L}_\beta^{(k)}(a,b)\, g(x_{\beta})\,,
\end{equation}
with:
\begin{equation}
\begin{array}{rcl}
\mathcal{L}_\beta^{(k)}(a,b)&=&\displaystyle\sum_{i=0\atop i\leq\beta}^k
  \theta(x-x_{\beta-i})\theta(x_{\beta-i+1}-x) \\
\\
&\times& \displaystyle\left[\prod^{k}_{m=0\atop m\ne
i}\frac{1}{x_{\beta}-x_{\beta-i+m}}\right]\left[\sum_{n=0}^k\frac{(-1)^n
         p_{\beta}^{(k)}(n)}{k-n+1}\left(b^{k-n+1}-a^{k-n+1}\right)\right]\,.
\end{array}
\end{equation}
