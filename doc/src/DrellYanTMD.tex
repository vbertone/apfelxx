...
f^{(3)}(\eta) & = &\displaystyle \frac{M \cosh(\eta-y)(Q^2 - 2
                    p_{T,\rm min}^{2} + 2 |\mathbf{q}_T|^2)- Q^{2} \sqrt{M^{2} \sinh(\eta-y)^{2} + p_{T,\rm min}^{2} }}{2 |\mathbf{q}_T| \left(M^{2} - p_{T,\rm min}^{2}\right)}\,, \\
\\
...
\int_{-\eta_{\rm max}}^{\eta_{\rm max}}d\eta\int_{\mbox{max}[f^{(2)}(\eta),-1]}^{\mbox{min}[f^{(3)}(\eta),f^{(4)}(\eta),1]}dx\dots\,.
\end{equation}
