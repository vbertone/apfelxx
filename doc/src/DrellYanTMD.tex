...
Eq.~(\ref{eq:weights}) can be decomposed as:
\begin{equation}
...
A very useful appealing of the computation of cross section in terms
of precomputed table as in Eqs.~(\ref{eq:finalinterpolated}) and
~(\ref{eq:finalinterpolatedfl}) is the fact that it exposes the free
parameters of the non-perturbative functions. To be more precise, the
non-perturbative function $f_{\rm NP}$, on top of being a function of
$x$, $b$, and $\zeta$, depends on a set of $N_p$ parameters
$\{\theta_k\}$, $k=1,\dots,N_p$, that are typically determined by fits
to data, in other words:
\begin{equation}
f_{\rm NP}\equiv f_{\rm NP}\left(x,b,\zeta;\{\theta_k\}\right)\,.
\end{equation}
Now, when performing a fit it is very useful to be able to compute the
derivative of the figure of merit (usually the $\chi^2$) with respect
to the parameters to be determined. In turn, this immediately implies
being able to compute the derivative of the observables. Referring to
Eq.~(\ref{eq:finalinterpolated}), the relevant quantity is:
\begin{equation}
...
\section{Cross section differential in $x_F$}
...
\begin{equation}
  ...
  \frac{2Q}{\sqrt{s}}\sinh y\,,
\end{equation}
such that:
\begin{equation}
  y(x_F,Q) =
  \sinh^{-1}\left(\frac{x_F\sqrt{s}}{2Q}\right) = \ln\left(\frac{x_F\sqrt{s}}{2Q}+\sqrt{1+\left(\frac{x_F\sqrt{s}}{2Q}\right)^2}\right)\,,
\end{equation}
and so that:
\begin{equation}
\frac{dx_F}{dy} = \frac{2Q}{\sqrt{s}}\cosh y\,.
\end{equation}
Therefore:
\begin{equation}
  \frac{d\sigma}{dQ dx_F dq_T} =
  \left.\frac{dy}{dx_F}\frac{d\sigma}{dQ dy dq_T}\right|_{y=y(x_F,Q)}=
  \left.\frac{\sqrt{s}}{2Q\cosh y}\frac{d\sigma}{dQ dy dq_T}\right|_{y=y(x_F,Q)}
\end{equation}
and, when integrating over $Q$ for a fixed value of $x_F$, one needs
to take into account the dependence of $y$ on $Q$.
