...
the function $g$. To do so, we need to compute the indefinite
integral:
\begin{equation}
G_i^{(k)}(x)=\int^xdy\,\ell_i^{(k)}(y) =\prod^{k}_{m=0\atop m\ne
i}\frac{1}{x_{\alpha+i}-x_{\alpha+m}}\int^x dy\prod^{k}_{m=0\atop m\ne
i}(y-x_{\alpha+m})\,.
\end{equation}
...
such that:
\begin{equation}
G_i^{(k)}(x) =\left[\prod^{k}_{m=0\atop m\ne
i}\frac{1}{x_{\alpha+i}-x_{\alpha+m}}\right]\left[\sum_{n=0}^k\frac{(-1)^n p_{\alpha+i}^{(k)}(n)}{k-n+1}x^{k-n+1}\right]\,.
\end{equation}
...
The integral of the function $g$ between $a$ and be can then be
computed on the grid as:
\begin{equation}
\int_a^bdy\,g(y) = \sum_{\beta=0}^{N_x+k-1}\left[\mathcal{G}_\beta^{(k)}(b) -\mathcal{G}_\beta^{(k)}(a)\right]g(x_{\beta})\,,
\end{equation}
with:
\begin{equation}
\mathcal{G}_\beta^{(k)}(x)= \sum_{j=0\atop j\leq\beta}^k
  \theta(x-x_{\beta-j})\theta(x_{\beta-j+1}-x)\left[\prod^{k}_{\delta=0\atop \delta\ne
j}\frac{1}{x_{\beta}-x_{\beta-j+\delta}}\right]\left[\sum_{\gamma=0}^k\frac{(-1)^\gamma
         p_{\beta}^{(k)}(\gamma)}{k-\gamma+1}x^{k-\gamma+1}\right]\,.
\end{equation}
