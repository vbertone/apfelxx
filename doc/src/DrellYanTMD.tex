...
\end{equation}
Using Eq.~(\ref{generalCase2}), we find that:
\begin{equation}
  I(a,b) =
  \sum_{\beta=0}^{N_x+k-1}\mathcal{G}_\beta^{(k)}(a,b) g(x_{\beta})\,,
\end{equation}
where:
% \begin{equation}
...
\sum_{i=0}^{{\rm min}(k,\beta)}
  \int_a^bdy\,\theta(y-x_{\beta-i})\theta(x_{\beta-i+1}-y)
  \prod^{k}_{m=0\atop m\ne
    i}\frac{y-x_{\beta-i+m}}{x_{\beta}-x_{\beta-i+m}}\,.
\end{equation}
Now, assuming that $a\leq b$, we can write the integral above as:
\begin{equation}
\int_a^bdy\,\theta(y-x_{\beta-i})\theta(x_{\beta-i+1}-y)\dots
...
\mathcal{G}_\beta^{(k)}(a,b) = \sum_{i=0}^{{\rm min}(k,\beta)}
  \theta(b-x_{\beta-i})\theta(x_{\beta-i+1}-a)\prod^{k}_{m=0\atop m\ne
    i}\frac{1}{x_{\beta}-x_{\beta-i+m}} \sum_{n=0}^k (-1)^n p_{\beta}^{(k)}(n) \int_{{\rm max}(a,x_{\beta-i})}^{{\rm min}(b,x_{\beta-i+1})}dy\,y^{k-n}\,.
\end{equation}
Fo the moment we leave the integral unsolved, we will come back on
that later because the situation is more complicated than that. What
is left to do is to determine the coefficients $p_{\beta}^{(k)}$. For
convenience, let us define the set
