...
into account kinematic constraints. Using the transverse metric
$g_\perp^{\mu\nu}$:
\begin{equation}
...
Eq.~(\ref{eq:deltaargument}). Finally, to perform the integral over $\phi$ we
use Eq.~(\ref{eq:intoverphi}) and we get:
\begin{equation}
\frac{d\mathcal{P}}{d|\mathbf{p}_T|d\eta} = \frac {3|\mathbf{p}_T|}{4\pi Q^2}
 \frac{Q^2+4 |\mathbf{p}_T|^2 \sinh^2(\eta-y)}{\sqrt{4|\mathbf{p}_T|^2|\mathbf{q}_T|^2-(Q^2-2|\mathbf{p}_T|M\cosh(\eta-y))^2}}\,.
\end{equation}
This allows us to get the Drell-Yan cross section differential in the
leptonic variables $|\mathbf{p}_T|$ and $\eta$:
\begin{equation}
\begin{array}{rcl}
  \displaystyle \frac{d\sigma}{dQ dy dq_T d\eta dp_T} &=&\displaystyle
                                                          \left[\frac
                                                          {3p_T}{4\pi
                                                          Q^2}\frac{Q^2+4 p_T^2\sinh^2(\eta-y)}{\sqrt{4p_T^2q_T^2-(Q^2-2p_TM\cosh(\eta-y))^2}}\right]\frac{d\sigma}{dQ dy dq_T}\\
\\
&=&\displaystyle
 \frac{4\alpha^2q_T p_T}{3 Q^5}\frac{Q^2+4
                                                         \sinh^2(\eta-y)p_T^2}{\sqrt{4p_T^2q_T^2-(Q^2-2p_TM\cosh(\eta-y))^2}}\\
\\
&\times&\displaystyle
  H(Q,\mu) \sum_q C_q(Q)
  \int\frac{d^2\mathbf{b}}{4\pi} e^{i \mathbf{b}\cdot \mathbf{q}_T}
         \overline{F}_q(x_1,\mathbf{b};\mu,\zeta)
         \overline{F}_{\bar{q}}(x_2,\mathbf{b};\mu,\zeta)\,.
\end{array}
\end{equation}
