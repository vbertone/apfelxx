...
\begin{equation}\label{eq:RGEproto}
...
because it underlies all others, is the strong coupling
$a_s=\alpha_s/4\pi$ that obeys the RGE\footnote{Notice that
  $\overline{\beta}$ in Eq.~(\ref{eq:RGEalphas}) is related to the
  usual QCD $\beta$-function by $\overline{\beta}=-\beta/a_s$.}:
\begin{equation}\label{eq:RGEalphas}
...
anomalous dimension:
\begin{equation}\label{eq:pertanstz}
a_s(\mu) = a_s(\mu_0) \sum_{n=0}^{k}b_n(\mu) a_s^n(\mu_0)\,.
\end{equation}
Plugging Eq.~(\ref{eq:pertanstz}) into both left- and right-hand side
of Eq.~(\ref{eq:RGEalphas}), one finds the following recursive
equations for the first few coefficients $b_n$:
\begin{equation}
\begin{array}{lcl}
\displaystyle \frac{d b_0}{d \ln \mu} &=& 0 \\
\\
\displaystyle \frac{d b_1}{d \ln \mu} &=& \beta^{(0)}b_0^2 \\
\\
\displaystyle \frac{d b_2}{d \ln \mu} &=& 2 \beta^{(0)} b_0 b_1 +
                                          \beta^{(1)} b_0^3\\
\\
\displaystyle \frac{d b_3}{d \ln \mu} &=& \beta^{(0)} (b_1^2 + 2 b_0
                                          b_2) + 3 \beta^{(1)} b_0^2
                                          b_1 + \beta^{(2)} b_0^4 \\
\\
\vdots
...
\begin{equation}\label{eq:pertcoefs}
\begin{array}{lcl}
b_0 &=& 1 \\
\\
b_1 &=& \displaystyle \beta^{(0)}\ln\left(\frac{\mu}{\mu_0}\right) \\
\\
b_2 &=& \displaystyle \left(\beta^{(0)}\right)^2\ln^2\left(\frac{\mu}{\mu_0}\right) + \beta^{(1)}\ln\left(\frac{\mu}{\mu_0}\right)\\
\\
b_3 &=& \displaystyle \left(\beta^{(0)}\right)^3\ln^3\left(\frac{\mu}{\mu_0}\right) + \frac{5}{2}\beta^{(0)}\beta^{(1)}\ln^2\left(\frac{\mu}{\mu_0}\right) + \beta^{(2)}\ln\left(\frac{\mu}{\mu_0}\right)  \\
\\
\vdots
\end{array}
\end{equation}
...
$\mu\ll \mu_0$, $b_n\sim \ln^{n}(\mu/\mu_0)\gg 1$. Therefore, despite the
...
\begin{equation}\label{eq:pertanstzinf}
a_s(\mu) = a_s(\mu_0) \sum_{n=0}^{\infty}b_n(\mu) a_s^n(\mu_0)\,.
\end{equation}
The coefficients $b_n$ can be computed iteratively exactly as shown
above for any value of $n$. The calculation above can be used to infer
the structure of the coefficients $b_n$:
\begin{equation}
  b_n(\mu) = \sum_{i=n-k}^n b_{n,i}\ln^i\left(\frac{\mu}{\mu_0}\right)\,,
\end{equation}
where $b_{n,i}$ are numerical coefficients given by combinations of
the coefficients $\beta_j$, $j=0,\dots,k$. Using this equation and
rearranging the series finally gives:
\begin{equation}\label{eq:pertanstzinfrearr}
  a_s(\mu) = a_s(\mu_0) \sum_{l=0}^k a_s^{l}(\mu_0) \sum_{n=0}^{\infty} b_{n+l,n}\left[a_s(\mu_0)\ln\left(\frac{\mu}{\mu_0}\right) \right]^{n}\,.
\end{equation}
Provided to be able to know the coefficients $b_{n+l,n}$ for all
$n$'s, this rearrangment exposes the resummation procedure: for each
power of $\alpha_s$ at which the $\beta$-function is known, all powers
of the kind $(a_s\ln(\mu/\mu_0))^n$ are included. In jargon, one
usually says that the knowledge of the anomalous dimension to order
$k$ allows N$^k$LL resummation. The question that remains to be
answered is how do we calculate all the $b_{n+l,n}$ coefficients to
effectively enable resummation. This is usually done by observing that
Eq.~(\ref{eq:RGEalphas}) with $k=0$ can be easily solved in a closed
form and gives:
\begin{equation}\label{eq:llsol}
...
$a_s$:
\begin{equation}\label{eq:transcend}
...
\end{equation}
This equation tells us that the logarithm appearing in the r.h.s.,
being proportional to $\alpha_s$ can be expanded and truncated to
leading order. Notice that this operation is allowed because no
potentially large logarithms of the scales are present. Therefore, we
can use Eq.~(\ref{eq:llsol}) inside this term, obtaining:
\begin{equation}\label{eq:firstexp}
...
\begin{equation}\label{eq:ansolut}
a_s^{\rm NLL}(\mu)=a_s^{\rm LL}(\mu) \left[1 + b_1  a_s^{\rm LL}(\mu) \ln\left(\frac{a_s(\mu_0)}{a_s^{\rm LL}(\mu)}\right)\right]\,,
\end{equation}
which gives the NLL solution for the evolution of the strong
coupling. The same kind of procedure can be applied iteratively to
obtain perturbatively more accurate solutions. However, it is now
important to stress an important aspect that is central to this
section, \textit{i.e.} the difference between
...
introducing subleading terms in the evolution equation. We point out
that the introduction of these spurious subleading terms has the only
scope to obtain a closed-form analytic expression for the evolution of
the coupling. Moreover, Eq.~(\ref{eq:ansolut}) has the additional
feature that it makes the evolution of the coupling
...
value. This is clearly not the case if one uses the (numerical)
solution of Eq.~(\ref{eq:transcend}). As an example,
Fig.~\ref{fig:Alphas} displays the behaviour of the NLL analytical
solution with $n_f=5$ active flavours evolved from
$\alpha_s(M_Z)=0.118$ down to 1 GeV and then up back to $M_Z$. As
clear, the backward and forward evolution curves do not coincide
leading to a mismatch of the value of $\alpha_s(M_Z)$.
\begin{figure}[h]
  \begin{centering}
    \includegraphics[width=0.6\textwidth]{plots/Alphas}
...
would do. Therefore, it appears convenient to use an exact solution of
...
evolution equations in Eq.~(\ref{eq:eveqs}). Another possibility is
that of attempting to solve the integral analytically. This turns out
to be possible provided that one uses the appropriate analytic
solution for the running of $\alpha_s$. As discussed above, this comes
at the price that the result violates the equations in
Eq.~(\ref{eq:eveqs}) by subleading terms. As we will see below, this
very violation gives origin to the resummation scale.
