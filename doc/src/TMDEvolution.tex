...
\begin{array}{l}
  ...
  -
  \gamma_K(\alpha_s(\mu))\ln\frac{\sqrt{\zeta}}{\mu}\right]\Delta-2P(\mu)\right\}\otimes
  C(\mu,\zeta)\,,
\end{array}
\end{equation}
...
expanded. In particular, if one assumes
...
% However, the assumption $\mu_0=\sqrt{\zeta_0}$ does not allow to
% identify the origin of the logarithms in Eq.~(\ref{eq:MatchExp}):
% whether they are logs of $\mu_0$ or $\zeta_0$. In order, to
% disentangle the two categories of logarithms and identify the
% respective coefficients, we generalise Eq.~(\ref{eq:MatchExp}) as:
% \begin{equation}
%   C(\mu_0,\mu_0^2) = \sum_{n=0}^{\infty}\left(\frac{\alpha_s(\mu_0)}{4\pi}\right)^{n}\sum_{m=0}^{2n}C^{(n,m)}\ln^m\left(\frac{\mu_0}{\mu_b}\right)\,.
% \end{equation}
% such that:
% \begin{equation}
% \begin{array}{rcl}
%   C(\mu_0,\mu_b^2) &=&\displaystyle 
%   \sum_{n=0}^{\infty}\left(\frac{\alpha_s(\mu_0)}{4\pi}\right)^{n}\sum_{i=0}^{n}C^{(n;i,0)}\ln^i\left(\frac{\mu_0}{\mu_b}\right)\,,\\
% \\
%   C(\mu_b,\zeta_0) &=&\displaystyle 
%   \sum_{n=0}^{\infty}\left(\frac{\alpha_s(\sqrt{\zeta_0})}{4\pi}\right)^{n}\sum_{j=0}^{n}C^{(n;0,j)}\ln^j\left(\frac{\sqrt{\zeta_0}}{\mu_b}\right)\,,
% \end{array}
% \end{equation}
% and then use the equations in Eq.~(\ref{eq:CEvEq}) one by one.
...
% Variation of the scale $\zeta$ around $Q^2$ are in principle also
% possible. However, such a variation only accounts for a different
% balance between the first term (that proportional to $K$) and the
% second term (the integral) in the argument of the exponent in
% Eq.~(\ref{eq:evkernelexp}).
