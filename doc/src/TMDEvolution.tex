...
{b}_T,\mu,\zeta) F(x, b_*({b}_T),\mu,\zeta)\,.
\end{equation}
This separation is advantageous because, due to the behaviour of
$b_*(b_T)$, $F(x, b_*({b}_T),\mu,\zeta)$ can be computed in
perturbation theory, while $f_{\rm NP}(x, {b}_T,\mu,\zeta)$ embodies
the non-perturbative dependence. It is important to stress that this
...
H_{ij}(Q) = \sum_{n=0}^\infty a_s^n(Q) H_{ij}^{(n)}=1+a_s(Q) H_{ij}^{(1)}+a_s^2(Q) H_{ij}^{(2)}+\mathcal{O}(a_s^3)\,,
\end{equation}
...
gives:
\begin{equation}
B_{ij,kl}^{(0,0)}(x,z) = \delta_{ik}\delta_{jl}\delta(1-x)\delta(1-z)\,.
\end{equation}
...
\begin{equation}
\begin{array}{rcl}
B_{ij,kl}^{(1,0)}(x,z) &=& \displaystyle H_{ij}^{(1)}\delta_{ik}\delta(1-x) \delta_{jl}\delta(1-z)+\delta_{ik}\delta(1-x)\mathbb{C}_{jl}^{(1,0)}(z)+\mathcal{C}_{ik}^{(1,0)}(x)\delta_{jl}\delta(1-z)\,,\\
\\
B_{ij,kl}^{(1,1)}(x,z) &=& \displaystyle S^{(1,1)}\delta_{ik}\delta(1-x) \delta_{jl}\delta(1-z)-\delta_{ik}\delta(1-x)\mathbb{P}_{jl}^{(0)}(z)-\mathcal{P}_{ik}^{(0)}(x)\delta_{jl}\delta(1-z)\,,\\
\\
B_{ij,kl}^{(1,2)}(x,z) &=& \displaystyle S^{(1,2)}\delta_{ik}\delta(1-x) \delta_{jl}\delta(1-z)\,.
\end{array}
\end{equation}
...
\begin{array}{rcl}
  ...
  \\
  {B}_{ij,kl}^{(2,1)}(x,z) &=&\displaystyle \left(\widetilde{S}^{(2,1)}+H_{ij}^{(1)}S^{(1,1)}\right)
                              \delta_{ik}\delta(1-x) \delta_{jl}\delta(1-z)\\
  \\
                          &-& \displaystyle
...
\\
 &-&\displaystyle \mathcal{P}_{ik}^{(0)}(x)\mathbb{C}_{jl}^{(1,0)}(z)
     -\mathcal{C}_{ik}^{(1,0)}(x)\mathbb{P}_{jl}^{(0)}(z)\\
\\
                              &+&\displaystyle \delta_{jl}\delta(1-z) \left(-\mathbb{P}^{(1)}(z)-\sum_\beta
                              \mathbb{C}_{j\beta}^{(1,0)}(z)\otimes
                              \mathbb{P}_{\beta l}^{(0)}(z)+\beta_0\mathbb{C}_{jl}^{(1,0)}(z) - \beta_0
    \mathbb{P}_{jl}^{(0)}(z)\right) \,,\\
  \\
  {B}_{ij,kl}^{(2,2)}(x,z) &=&\displaystyle \left(\widetilde{S}^{(2,2)}+H_{ij}^{(1)}S^{(1,2)}\right)
                              \delta_{ik}\delta(1-x) \delta_{jl}\delta(1-z)\\
  \\
                          &+&\displaystyle
  ...
  \\
                          &-&\displaystyle
                              S^{(1,1)}\left[\mathcal{P}_{ik}^{(0)}(x)\delta_{jl}\delta(1-z)+\delta_{ik}\delta(1-x)\mathbb{P}_{jl}^{(0)}(z)\right]\\
  \\
                          &+& \displaystyle \frac12 \sum_{\alpha}\mathcal{P}^{(0)}_{i\alpha}(x)\otimes
    \mathcal{P}_{\alpha k}^{(0)}(x)\delta_{jl}\delta(1-z) + 
                              \mathcal{P}_{ik}^{(0)}(x)\mathbb{P}_{jl}^{(0)}(z)
                              +\delta_{ik}\delta(1-x) \frac12 \sum_{\beta}\mathcal{P}^{(0)}_{j\beta}(z)\otimes
    \mathbb{P}_{\beta l}^{(0)}(z) \,,\\
  \\
  {B}_{ij,kl}^{(2,3)}(x,z) &=&\displaystyle
                              \widetilde{S}^{(2,3)}\delta_{ik}\delta(1-x) \delta_{jl}\delta(1-z)
                              - S^{(1,2)}\left[\mathcal{P}_{ik}^{(0)}(x)\delta_{jl}\delta(1-z)
                              +
                              \delta_{ik}\delta(1-x)\mathbb{P}_{jl}^{(0)}(z)\right]\,,\\
  \\
  {B}_{ij,kl}^{(2,4)}(x,z) &=&\displaystyle \widetilde{S}^{(2,4)}\delta_{ik}\delta(1-x) \delta_{jl}\delta(1-z) \,.
\end{array}
\end{equation}
...
\begin{equation}\label{eq:IkNonPert}
  ...
  {b}_T,Q,Q^2) f_{\rm NP}^{(2)}(z,
  {b}_T,Q,Q^2)\ln^p\left(\frac{b_*^2(b_T)Q^2}{4e^{-2\gamma_E}}\right)\,,
\end{equation}
...
Eq.~(\ref{eq:IkNonPert}) cannot be computed analytically. However, in
the case $k=0$ Eq.~(\ref{eq:IkNonPert}) has an intersting
...
differential in $q_T$ fot $q_T>0$, \textit{i.e.} without the
$\delta(q_T)$ terms, and with no non-perturbative effect. Considering
that, $S^{(1,1)} = 6C_F$ and $S^{(1,1)} = -2C_F$, this yields:
\begin{equation}\label{eq:asymptfromres}
\begin{array}{rcl}
  \displaystyle  \frac{d\sigma}{dq_T^2} &\propto &\displaystyle a_s(Q)\sum_{ij,kl}\left[
  ...
  \\
                                        &=&\displaystyle
                                            \frac{a_s(Q)}{q_T^2}\sum_{ij,kl}\bigg[4C_F\left(\ln\left(\frac{Q^2}{q_T^2}\right)-\frac{3}{2}\right)\delta_{ik}\delta_{jl}\delta(1-x)\delta(1-z)\\
  \\
                                        &+&\displaystyle
                                            ...
                                            d_l(z,Q)\\
  \\
                                        &=&\displaystyle \frac{a_s(Q)}{q_T^2}\sum_{ij}\bigg[4C_F\left(\ln\left(\frac{Q^2}{q_T^2}\right)-\frac{3}{2}\right) f_i(x,Q)d_j(z,Q)\\
  \\
                                        &+&\displaystyle \left(\sum_{k}\mathcal{P}_{ik}^{(0)}(x) \mathop{\otimes}_x
                                            f_k(x,Q)\right)d_j(z,Q) + f_i(x,Q)\left(\sum_l\mathbb{P}_{jl}^{(0)}(z) \mathop{\otimes}_zd_l(z,Q)\right) +\mathcal{O}(a_s^2)\,.
\end{array}
\end{equation}
This result, up to some pre-factors and to selecting the right flavour
...
$(1-x)^{-1}$ and $(1-z)^{-1}$\footnote{This step has the effect of
  removing the uncancelled soft divergencies.}, one can see that up to
