...
be generated by expanding $\alpha_s(\mu_0)$ around $\alpha_s(\mu_b)$
using the appropriate RGE. Of course, such expansion is reliable only
if $\mu_0$ and $\mu_b$ are comparable such not to generate large
logarithms. The second term in the r.h.s. of Eq.~(\ref{eq:CuspEv})
instead takes care, through the evolution of $\alpha_s$, of resumming
large logarithms in the case in which $\mu\gg \mu_0$. The coefficients
$K^{(n,m)}$ up to $n=3$ are reported in Eq.~(D.9) of
Ref.~\cite{Echevarria:2016scs} and up to $n=2$ in Eq.~(69) of
Ref.~\cite{Collins:2017oxh}. They differ by a factor $-2$ due to a
different definition of $K$. In addition, the logarithmic expansion is
done in terms of $\ln(\mu_0/\mu_b)$ in Ref.~\cite{Collins:2017oxh} and
in terms of $\ln(\mu_0^2/\mu_b^2)$ in
...
to determine the evolution of the matching function $C$. If initial
and final scales are not too far apart, the solution can be
expanded. In particular, if $\mu_0=\sqrt{\zeta_0}\simeq \mu_b$ in
...
final pairs of scales, $(\mu_0,\zeta_0)$ and $(\mu,\zeta)$. The
initial scales are usually identified with $\mu_b$ up to a small
factor $C_i$, \textit{i.e.}
$(\mu_0,\zeta_0) = (C_i\mu_b,C_i^2\mu_b^2)$, with
$\mu_b = 2e^{-\gamma_E}/b_T$. This is advantageous because the
...
small enough not to spoil their perturbative convergence. Variations
of $C_i$ around unity can be used to estimate the impact of
...
$f_{\rm NP}$ in Eq.~(\ref{eq:separatation}) does not depend on the
renormalisation scale $\mu$ is that this dependence cancels in the
ratio. To be more specific, if one chooses
...
required behaviour $b_*$, if $b_T$ becomes small, $b_T$ and $b_*$ get
closer and so do $\mu_{b_*}$ and $\mu_b$ as well as $\zeta_{b_*}$ and
$\zeta_b$. Therefore, from the equation above, we see that
$f_{\rm NP}\rightarrow 1$ for $b_T\rightarrow 0$. Conversely, if $b_T$
becomes large $b_*$ saturates to $b_{\rm max}$. In this limit, we can write:
\begin{equation}
\begin{array}{l}
\displaystyle  f_{\rm NP}(x,
...
% In order to clarify this discrepancy, in the next section we integrate
% the expressions of Ref.~\cite{deFlorian:1997zj} over $x$ with the aim
% of recovering the hard cross sections for the single-inclusive
% production of the hadron $h$ in $e^+e^-$ annihilation (SIA).

% \section{Integrating over $x$}

% In order to compute the SIA cross section, we first need to integrate
% over $y$. To do so we need to know that:
% \begin{equation}
% \int_0^1dy\,Y_+ = \frac{4}{3}\quad\mbox{and}\quad \int_0^1dy\,y^2 =
% \frac{1}{3}\,,
% \end{equation}
% so that:
% \begin{equation}\label{eq:intxsec}
% \frac{d\sigma^h}{dx dz} = \int_0^1 dy \,\frac{d\sigma^h}{dx dy dz} = \frac{2\pi\alpha^2}{xyQ^2}\left[\frac43F_{2}-\frac{1}{3}F_{L}\right]
% \end{equation}
