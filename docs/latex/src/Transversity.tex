...
longitudinally polarised collinear distributions, the gluon
distribution, that in fact corresponds to a linearly polarised gluon,
decouples and evolves multiplicatively. This simplifies the structure
of the evolution equations that, when written in the evolution basis,
...
DGLAP evolution equations for the transversity distributions reduces to
the following set of \textit{decoupled} integro-differential equation:
\begin{equation}
\begin{array}{rcl}
\displaystyle \mu^2\frac{dg}{d\mu^2} &=& \displaystyle P_{gg}\otimes g\,,\\
\\
...
\end{equation}
Let us first focus on the quark distributions. The splitting functions
$P_{qq}$, $P^V$, $P^+$, and $P^-$ are usually decomposed as follows:
\begin{equation}
\begin{array}{l}
...
\begin{equation}
P_{qq}^{V,(0)}(y) =
2C_F\left[-2+2\left(\frac{1}{1-y}\right)_++\frac32\delta(1-y)\right]\,.
...
4C_F\left(C_F-\frac12C_A\right)\left[ - 1 + y
  -\frac{4S_2(y)}{1+y}\right]\,,
...
We now move to considering the evolution of the linearly polarised
gluon distribution that also evolves multiplicatively. In the
following, we will only consider the gluon PDF and leave aside the
gluon FF for which, to the best of my knowledge, the NLO correction to
the splitting function has not been computed yet. The leading-order
splitting function is given in Ref.~\cite{Artru:1989zv}:
\begin{equation}
P_{gg}^{(0)}(y) =
2C_A\left[\left(\frac{2}{1-y}\right)_+-2+\delta(1-y)\left(\frac{11-4n_f
    T_R}{6C_A}\right)\right]\,.
\end{equation}
The NLO correction has been computed in Ref.~\cite{Vogelsang:1998yd}
and reads:
\begin{equation}
\begin{array}{rcl}
\displaystyle P_{gg}^{(1)}(y) &=& \displaystyle \Bigg\{4 C_A^2\left[\frac{y\ln^2y-4y\ln
  y\ln(1-y)}{1-y}+\frac{1-y^3}{6y}+S_2(y)
    \left(\frac{2}{1+y}-2\right) -\frac{67}{9}+\frac{\pi^2}3\right]\\
\\
&+&\displaystyle 4n_f T_RC_A\left(\frac{20}9+\frac{1-y^3}{3y}\right) -8 n_f T_R C_F\frac{1-y^3}{3y}\Bigg\}\\
\\
&+&\displaystyle \Bigg\{2C_A^2\left(\frac{134}{9}-\frac{2\pi^2}3\right) -\frac{80}9n_f C_AT_R\Bigg\}\left(\frac{1}{1-y}\right)_+\\
\\
&+&\displaystyle \Bigg\{4 C_A^2\left(\frac{8}{3}+3\zeta_3\right) -\frac{16}3n_f T_RC_A -4 n_f T_R C_F\Bigg\}\delta(1-y)\,.
\end{array}
\end{equation}
