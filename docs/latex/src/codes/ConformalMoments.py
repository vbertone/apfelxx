...
def a(n, k):
    l = n - 2 * k
    return (-1)**k * 2**l * gamma(l + k + 3./2.) / gamma(3./2.) / factorial(k) / factorial(l)
...
def Icom(n, z):
    ...
    for k in range(math.floor(n/2)+1):
        l = n - 2 * k
        summation += a(n, k) * ( sum([ ( 1. / ( l + 2 ) + 1. / ( l + 1 ) - 2. / ( l - j ) ) * ( 1 + (-1)**(l-j) ) * z**j / 2. for j in range(l)]) + ( 1. / ( l + 2 ) + 1. / ( l + 1 ) + 2 * sum([ 1. / ( j + 1 ) for j in range(l)]) ) * z**l )
...
def Icom2(n, z):
    ...
    for h in range(math.floor(n/2)+1):
        ...
        terms.extend([ - a(n, h-j) / a(n, h) * ( 1. / ( n - 2 * h + 2 * j + 2 ) + 1. / ( n - 2 * h + 2 * j + 1 ) - 1. / j ) for j in range(1,h+1)])
        coeffs.append(terms)
        #print(h, terms)
    for h in range(math.floor(n/2)+1):
        print(sum(coeffs[h]), V(n))
    return sum([a(n, h) * z**(n-2*h) * sum(coeffs[h]) for h in range(math.floor(n/2)+1)])
