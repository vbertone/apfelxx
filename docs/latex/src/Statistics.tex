...
can be found in other codes, such as {\tt NangaParbat} (see
...
instructive to compute the \textit{systematic shifts} generated by the
...
\end{pmatrix}\,.
\end{equation}
In addition, the set of two theoretical predictions $\{t_i\}$ allows
one to define a column vector of residuals
...
can be regarded as an independent subset of the original set. However,
this is not the case when correlations are present.
...
see that Eq.~(\ref{eq:redchi21}) does not fulfil the same
...
meaningful only when including the systematic shifts introduced in
...
manifests itself into the DAB. Therefore, from this point of view, the
DAB is not a flaw of the fitting procedure but rather an indication
that the data set is intrinsically inconsistent. It thus appears
...
\end{equation}
Since $\overline{\mu}_0$ does not depend on $t_0$, all the following
