...
$f$ is the so-called damping function. Let us now consider the effect
on the structure functions associated with the $(n_f+1)$-th heavy
quark having mass $m_h$. The factorisation of these structure
functions thus reads as follows:
\begin{equation}
\begin{array}{l}
  \displaystyle F^{\rm ZM}(x,Q)=x\int_x^1\frac{dy}{y}C_j^{\rm ZM}\left(y,a_s^{(n_f+1)}(\mu_{\rm R}),\frac{\mu_{\rm F}}{Q},\frac{\mu_{\rm R}}{Q}\right)f_j^{(n_f+1)}\left(\frac{x}{y},\mu_{\rm F}\right)\,,\\
  \\
  \displaystyle F^{\rm M}(x,Q)=x\int_{\chi}^1\frac{dy}{y}C_i^{\rm M}\left(y,a_s^{(n_f)}(\mu_{\rm R}),\frac{m_h}{Q},\frac{\mu_{\rm F}}{Q},\frac{\mu_{\rm R}}{Q}\right)f_i^{(n_f)}\left(\frac{\chi}{y},\mu_{\rm F}\right)\,,\\
  \\
  \displaystyle F^{\rm M0}(x,Q)=x\int_x^1\frac{dy}{y}C_i^{\rm M0}\left(y,a_s^{(n_f)}(\mu_{\rm R}),\frac{m_h}{Q},\frac{\mu_{\rm F}}{Q},\frac{\mu_{\rm R}}{Q}\right)f_i^{(n_f)}\left(\frac{x}{y},\mu_{\rm F}\right)\,,
\end{array}
\label{eq:SFAllSchemes}
\end{equation}
where $\chi=x/\eta$ is the appropriate slow-rescaling variable
accounting for the heavy-quark mass. The superscript in the PDFs and
$a_s=\alpha_s/(4\pi)$ indicates that they are evolved using a scheme
with $n_f$ or $n_f+1$ active flavours.\footnote{The case in which
  there is more than one unit of difference between schemes can be
  addresses recursively including one heavy quark at the time.} Also,
a sum over the indices $i$ and $j$ is understood. However, we restrict
to the case in which the massive structure functions (M and M0)
receive no intrinsic heavy-flavour contributions, such that only
dynamical partons are present in the initial states. Therefore, while
$j$ is potentially allowed to run over \textit{all} possible partons,
$i$ is only allowed to run over the $n_f$ light partons. However,
below we will be expressing $f_i^{(n_f)}$ in terms of $f_j^{(n_f+1)}$
at some arbitrary scale. Since a general set $f_i^{(n_f+1)}$ would
generally give rise to heavy-quark contributions to $f_i^{(n_f)}$, the
only way to enforce that $i$ only runs ove light favours is to set
$C_i^{\rm M/M0}=0$ whenever $i$ corresponds to a heavy parton
(\textit{i.e.}  $C_h^{\rm M/M0}=0$, see below).\footnote{This
  assumption is not consistent but it simplifies the picture and is
  often employed, perhaps unknowingly, in actual implementations.} The
coefficient functions $C$ are perturbatively calculable and read:
\begin{equation}
C_i^{s}(y,a_s,\dots)=\sum_{l=0}a_s^lC_i^{s,(l)}(y,\dots)\,,\quad s={\rm
ZM, M, M0}\,.
\end{equation}
...
and $a_s$. As anticipated above, we want to express PDFs and $a_s$ in
the $n_f$-scheme which appear in $F^{\rm M}$ and $F^{\rm M0}$ in terms
of PDFs and $a_s$ in the $(n_f+1)$-scheme. To do so, we take $Q$ as
the transition scale between the two schemes and use the appropriate
matching conditions, which are implemented in {\tt APFEL++} as
follows:
\begin{equation}
  \begin{array}{l}
    \displaystyle a_s^{(n_f+1)}(Q)=a_s^{(n_f)}(Q)\left[1+\sum_{k=1}\left[a_s^{(n_f)}(Q)\right]^k M^{(k)}\left(\frac{m_h}{Q}\right)\right]\,,\\
\\
    \displaystyle f_j^{(n_f+1)}\left(z,Q\right)=\int_z^1\frac{dw}{w}\left[\delta_{ji}\delta(1-w)+\sum_{k=1}\left[a_s^{(n_f+1)}(Q)\right]^k A_{ji}^{(k)}\left(w,\frac{m_h}{Q}\right)\right]f_i^{(n_f)}\left(\frac{z}{w},Q\right)\,.\\
\end{array}
\label{eq:matchingconditions}
\end{equation}
Importantly, here we are allowing the index $i$ to also run over heavy
flavours. This is necessary to invert these equalities, and express
$a_s^{(n_f)}$ in terms of $a_s^{(n_f+1)}$ and $f_i^{(n_f)}$ in terms
of $f_i^{(n_f+1)}$. This can be done perturbatively, which, up to NNLO
accuracy, produces:
\begin{equation}
  \begin{array}{l}
    \displaystyle a_s^{(n_f)}=a_s^{(n_f+1)}\left[1-M^{(1)}a_s^{(n_f+1)} -\left(M^{(2)}-2(M^{(1)})^2\right) \left[a_s^{(n_f+1)}\right]^2+\mathcal{O}(\alpha_s^3)\right]\,,\\
\\
    \displaystyle f_i^{(n_f)}=\left[\delta_{ij}\delta(1-w)-a_s^{(n_f+1)}A_{ij}^{(1)}-\left[a_s^{(n_f+1)}\right]^2 \left(A_{ij}^{(2)}-A_{ik}^{(1)}\otimes A_{kj}^{(1)}\right)+\mathcal{O}(\alpha_s^3)\right]\otimes f_j^{(n_f+1)}\,,\\
\end{array}
\end{equation}
where we omitted all dependences and used the symbol $\otimes$ to
indicate Mellin convolutions. Note that the second equation generally
produces heavy-quark PDFs in the $n_f$-scheme. However, they will not
contribute under the assumption $C_h^{\rm M/M0}=0$. These equalities
finally allow us to write:
\begin{equation}
\begin{array}{l}
  \displaystyle F^{\rm M}(x,Q)=x\int_{\chi}^1\frac{dy}{y}B_j^{\rm M}\left(y,a_s^{(n_f+1)}(Q),\frac{m_h}{Q}\right)f_j^{(n_f+1)}\left(\frac{\chi}{y},Q\right)\,,\\
  \\
  \displaystyle F^{\rm M0}(x,Q)=x\int_x^1\frac{dy}{y}B_j^{\rm M0}\left(y,a_s^{(n_f+1)}(Q),\frac{m_h}{Q}\right)f_j^{(n_f+1)}\left(\frac{x}{y},Q\right)\,.
\end{array}
\label{eq:massiveSFafterSchemeChange}
\end{equation}
Note that, as opposed to Eq.~(\ref{eq:SFAllSchemes}) where the massive
structure functions only receive contributions from light flavours, in
Eq.~(\ref{eq:massiveSFafterSchemeChange}) $j$ runs also over heavy
flavours. The perturbative coefficients of the functions $B$ are given
by:
\begin{equation}
  \begin{array}{l}
    B_j^{(0)} = C_j^{(0)}\,,\\
\\
    B_j^{(1)} = C_j^{(1)}-C_i^{(0)}\otimes A_{ij}^{(1)}\,,\\
\\
    B_j^{(2)} = C_j^{(2)}-C_i^{(1)}\otimes \left(A_{ij}^{(1)}+M^{(1)}\delta_{ij}\delta(1-y)\right)-C_i^{(0)}\otimes\left(A_{ij}^{(2)}-A_{ik}^{(1)}\otimes A_{kj}^{(1)}\right)\,,\\
\\
\dots
  \end{array}
\end{equation}
which hold for both M and M0 schemes. Let us then write explicitly the
coefficient functions $B_j^{(n)}$ for the gluon ($j=g$), for light
flavours ($j=l\leq n_f$), and for heavy flavours ($j=h>n_f$). To do
so, we use the fact that $C_j^{(0)}=a_j\delta_{jl}\delta(1-y)$, with
$a_j$ some appropriate charge, which results in:
\begin{equation}
  \begin{array}{l}
    B_j^{(0)} = a_j\delta_{jl}\delta(1-y)\,,\\
\\
    B_j^{(1)} = C_j^{(1)}-a_jA_{lj}^{(1)}\,,\\
\\
    B_j^{(2)} = C_j^{(2)}-C_i^{(1)}\otimes \left(A_{ij}^{(1)}+M^{(1)}\delta_{ij}\delta(1-y)\right)-a_j\left(A_{lj}^{(2)}-A_{lk}^{(1)}\otimes A_{kj}^{(1)}\right)\,,\\
\\
\dots
  \end{array}
\end{equation}
We also know that $A_{lk}^{(1)}=A_{kl}^{(1)}= A_{lg}^{(2)}= A_{lh}^{(2)} = C_h^{(n)} =0$ and
that $A_{gg}^{(1)}=-M^{(1)}\delta(1-y)$, which finally gives:
\begin{equation}
\small
  \begin{array}{lll}
    \displaystyle
    ...
    B_l^{(1)} = C_l^{(1)}\,,\\
    \\
    B_l^{(2)} = C_l^{(2)}-M^{(1)}C_l^{(1)}-a_lA_{ll}^{(2)}\,,\\
    \\
    \dots
    \end{array} &
     \displaystyle\qquad   \begin{array}{l}
    B_g^{(0)} = 0\,,\\
\\
    B_g^{(1)} = C_g^{(1)}\,,\\
\\
...
\\
\dots
                     \end{array}&\displaystyle\qquad
  \begin{array}{l}
    B_h^{(0)} = 0\,,\\
\\
    B_h^{(1)} = 0\,,\\
\\
    B_h^{(2)} = -C_g^{(1)}\otimes A_{gh}^{(1)}\,,\\
\\
\dots
  \end{array}
  \end{array}
  \label{eq:explicitBCFs}
\end{equation}
With a totally arbitrary move, we set $B_h^{(n)}=0$, which finally
produces that:
\begin{equation}
\begin{array}{l}
  \displaystyle F^{\rm M}(x,Q)=x\int_{\chi}^1\frac{dy}{y}B_i^{\rm M}\left(y,a_s^{(n_f+1)}(Q),\frac{m_h}{Q}\right)f_i^{(n_f+1)}\left(\frac{\chi}{y},Q\right)\,,\\
  \\
  \displaystyle F^{\rm M0}(x,Q)=x\int_x^1\frac{dy}{y}B_i^{\rm M0}\left(y,a_s^{(n_f+1)}(Q),\frac{m_h}{Q}\right)f_i^{(n_f+1)}\left(\frac{x}{y},Q\right)\,.
\end{array}
\label{eq:massiveSFafterSchemeChangeNoHF}
\end{equation}
so that the massive structure functions do not receive any
contributions from heavy-quark PDFs also after scheme change. Again,
this matches usual implementations of the FONLL scheme.
...
ourselves to considering to electromagnetic structure functions $F_2$
and $F_L$, assuming that $F_3$, in the NC case, is only relevant at
large scales where heavy-quark mass effects are negligible. We now
split the structure functions into components associated to single
quark flavours. This is done by setting to zero all electric charges
except that of the flavour under consideration (see
Eq.~(\ref{eq:StructureFunctionSingle})). This enables the following
general decomposition:
\begin{equation}
F(x,Q)=\sum_{k=1}^{6}e_k^2F_k(x,Q)\,,
\end{equation}
Then we assume to have $n_f$ light flavours and while rest being
massive (typically, $d$, $u$, and $s$ are considered to be light,
while $c$, $b$, and $t$ heavy). This allows us to write:
\begin{equation}
F(x,Q)=\sum_{l=1}^{n_f}e_l^2F_l(x,Q)+\sum_{h=n_f+1}^{6}e_h^2F_h(x,Q)\,.
...
terms that depend on a single mass (this will become clearer
later). We point out that we will adopt the so-called ``absolute''
perturbative ordering, according to which massive and massless sectors
contribute with the same power of $\alpha_s$ at each perturbative
order. This is in contrast with the FONLL spirit, which typically
adopts the ``relative'' ordering, and more in line with the ACOT
spirit. However, the absolute ordering allows for a cleaner
implementation and effectively aligns with the usual implementation of
the FONLL scheme beyond NLO accuracy.\footnote{Using the FONLL
...
$f_j^+=f_j+f_{\overline{j}}$. This expression holds for each of the
structure functions making up the FONLL scheme with
$C=C^{\rm ZM}, B^{\rm M}, B^{\rm M0}$ and $j$ running over all
flavours in the ZM scheme and only over the $n_f$ light flavours in
the massive schemes.

With these premises, let us address $F_l$ and $F_h$ separately
starting with the former. The ZM structure function is:
\begin{equation}
F_l^{\rm ZM}=\theta(Q-m_l)\left[C_g^{\rm ZM}\otimes f_g +C_{\rm PS}^{\rm ZM}\otimes \sum_{j=1\atop j\neq l}^6f_j^++(C_{\rm PS}^{\rm ZM}+C_+^{\rm ZM})\otimes f_l^+\right]\,,
\label{eq:ZMlightSF}
\end{equation}
