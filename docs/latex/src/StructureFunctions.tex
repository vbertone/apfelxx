...
aspects of the computation, more ``theoretical'' aspects will be
touched such as the computation of scale variations and of the
...
pre-computation of the coefficient functions on $x$-space grid.  A
...
used.\footnote{Actually, if neglecting intrinsic heavy-quark
  ...
  coefficient function for each structure functions, available for
...
modern experiments is sizeable at large $Q$ only where mass effects
are negligible.
...
Ref.~\cite{Gluck:1996ve}. However, their implementation in {\tt
...
Despite most of the time the residual $R_f(x)$ function can be
evaluated analytically, sometimes it needs to be evaluated
numerically. The $+$-prescribed functions that present in the
expressions above give raise to the following residual functions that
...
\end{equation}
while the following integral:\footnote{Actually, also this integral
  can be computed analytically but it involves the di-log and I will
  leave it like this for now.}
\begin{equation}
  R(x)=-\int_0^xdz\frac{\ln(1-\lambda z)}{1-z}
\end{equation}
is computed numerically. The advantage of the $x$-prescription is
...
\label{expDGLAP2}
\end{equation}
It is also useful to consider $t_F=0$ in Eq.~(\ref{BETAexp}), that is
...
preferred choice in {\tt APFEL} is the so-called QCD evolution basis:
\begin{equation}
...
\end{equation}
We can now express the term in square brackets in terms $C_+$ and
...
It is useful to separate the contributions deriving from the coupling
of the vector boson to the different quark flavours. To do so, one
just needs to select the contributions proportional to, say, the
$k$-th charge $e_k^2$. This is easily done with the following
replacement:
\begin{equation}
...
F^l = \langle e_l^2 \rangle \left[C_g g +\left(C_{\rm PS} +\frac1{n_f}C_+
  \right)
  \Sigma\right]+\frac{1}{2}(e_u^2-e_d^2)C_+T_3+\frac{1}{6}(e_u^2+e_d^2-2e_s^2)C_+T_8+\langle
e_l^2 \rangle C_+\sum_{j=4}^{n_f} \frac{1}{j(j-1)} f_j\,.
\label{LightSF}
\end{equation}
...
\begin{equation}
F_3=\langle c_q^2 \rangle \frac1{n_f}C_- V+C_-\sum_{j=2}^{n_f} d_j
g_j\,,
...
matrix diagonalizes. In particular, up to $\mathcal{O}(\alpha_s^2)$,
one has that:
\begin{equation}
\begin{array}{ll}
...
section with the DIS one it is easy to realize that defining:
\begin{equation}
\begin{array}{l}
