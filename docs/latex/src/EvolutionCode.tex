...
\begin{equation}
\mu^{2}\frac{\partial q_{i}(x,\mu^{2})}{\partial
  \mu^{2}}=\int^{1}_{x}\frac{dy}y
P_{ij}\left(\frac{x}{y},\alpha_{s}(\mu^{2})\right)q_{j}(y,\mu^{2})
\label{dglap}
\end{equation}
...
\begin{equation}
\frac{\partial \tilde{q}_{i}(x,t)}{\partial t}=\int^{1}_{x}\frac{dy}y
\tilde{P}_{ij}\left(\frac{x}{y},t\right)\tilde{q}_{j}(y,t)
\label{dglap2}
\end{equation}
...
\begin{equation}
w_{\alpha}^{(k)}(x) = \sum_{j=0,j \leq
  \alpha}^{k}\theta(x-x_{\alpha-j})\theta(x_{\alpha-j+1}-x)\prod^{k}_{\delta=0,\delta\ne
  j}\left[\frac{x-x_{\alpha-j+\delta}}{x_{\alpha}-x_{\alpha-j+\delta}}\right]\,.
\label{LagrangeFormula}
\end{equation}
This automatically means that:
\begin{equation}
w_{\alpha}^{(k)}(x) \neq 0 \quad\mbox{for}\quad x_{\alpha-k} < x <
x_{\alpha+1}.  
\label{nonzero}
\end{equation}
...
w_{\alpha}^{(k)}(x) = \sum_{j=0,j \leq
  \alpha}^{k}\theta(x-x_{\alpha-j})\theta(x_{\alpha-j+1}-x)\prod^{k}_{\delta=0,\delta\ne
  j}\left[\frac{\ln(x)-\ln(x_{\alpha-j+\delta})}{\ln(x_{\alpha})-\ln(x_{\alpha-j+\delta})}\right]
...
we have that:
\begin{equation}
\Pi_{ij,\beta\alpha}(t) = \sum_{n=0}^{k} a_s^{n+1}(t)
\Pi_{ij,\beta\alpha}^{(n)}\,,
\label{splittingexp}
\end{equation}
...
\begin{equation}
x_{\alpha+t} < x \leq x_{\alpha+t+1}\quad\mbox{with}\quad t =
0,\dots,k-1\,,
...
\begin{equation}
g(x) =
\sum_{\alpha=0}^{N_x}\theta(x-x_{\alpha+t})\theta(x_{\alpha+t+1}-x)\sum_{i=0}^k
g(x_{\alpha+i})\prod^{k}_{m=0,m\ne
  i}\frac{x-x_{\alpha+m}}{x_{\alpha+i}-x_{\alpha+m}}\,,
