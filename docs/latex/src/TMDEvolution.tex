...
to two loops reads:
\begin{equation}\label{eq:alphasexp}
...
\begin{equation}\label{eq:Cscalevars}
\begin{array}{rcl}
...
\begin{equation}\label{eq:alphasscalvar}
\begin{array}{rcl}
a_s(\mu) &=&\displaystyle 
...
$\mathcal{O}\left(\alpha_s^n\right)$.
...
the momentum fraction $x$, and denoting by $\Gamma$ the evolution
...
and $\alpha_s$ corrections while N$^k$LL requires including
...
\end{array}
\end{equation}
The reason why this procedure gives potentially larger variations is
that the expansion of the evolution operator is done around the scale
$\mu_0\simeq \mu_b$. At this scale the coupling can become large and
so do the neglected higher-order corrections.
...
\begin{equation}\label{eq:DGLAPeqOpExp}
...
obtained by expanding the solutions of the evolution equantion between
...
\begin{equation}\label{eq:alphasexp2}
a_s(Q)= a_s(\mu_{\rm R})+a_s^2(\mu_{\rm R})\beta_0\ln\frac {\mu_{\rm R}^2}{Q^2} +\mathcal{O}(\alpha_s^3)\,,
\end{equation}
so that $a_s^2(Q)= a_s^2(\mu_{\rm R})+\mathcal{O}(\alpha_s^3)$. While
...
\begin{equation}\label{eq:expPDFsFFs}
\begin{array}{rcl}
...
virtual contributions. The net effect is the presence of the plus
prescription in all terms that behave as $(1-x)^{-1}$ and
$(1-z)^{-1}$. By doing so, one can see that up to ``local'' terms
(\textit{i.e.} those terms proportional to $\delta$-functions) the
expressions of Appendix~C of Ref.~\cite{deFlorian:1997zj} are
reproduced(\footnote{The expressions reported in
  Ref.~\cite{deFlorian:1997zj} are those computed long time ago in
  Ref.~\cite{Furmanski:1981cw}.}). The local terms correspond to
virtual corrections that thus give contributions at $q_T=0$. As such,
they should coincide with the terms proportional to $\delta(q_T^2)$
generated in the expansion of the resummed calculation. Specifically,
they should be equal to the coefficients ${B}_{ij,kl}^{(n,0)}$ in the
expansion in Eq.~(\ref{eq:xsecexp}). This is evidently the case for
${B}_{ij,kl}^{(0,0)}$ that gives the leading-order contribution to the
integrated cross section. Unfortunately, this is not true at
...
${B}_{ij,kl}^{(0,0)}$ summed over the indices of the hard function:
\begin{equation}
\begin{array}{rcl}
...
\begin{array}{rcl}
  \overline{C}_{qq}(x,z) &=& \displaystyle 2C_F\left[-8\delta(1-x)
                            \delta(1-z)+\delta(1-x)\left(1-z+2p_{qq}(z)\ln
                             z\right)+\left(1-x\right)\delta(1-z)\right]\,,\\
\\
  \overline{C}_{gq}(x,z) &=& \displaystyle
                             2C_F\left[\delta(1-x)\left(z+2p_{gq}(z)\ln
...
\begin{equation}
\begin{array}{rcl}
p_{qq}(x) &=&\displaystyle  \frac{1+z^2}{1-z}\,,\\
\\
p_{gq}(x) &=&\displaystyle  \frac{1+(1-z)^2}{z}\,,\\
\\
p_{qg}(x) &=&\displaystyle  1-2z(1-z)\,.
\end{array}
\end{equation}
...
\begin{equation}\label{eq:approxLL}
...
\begin{equation}\label{eq:expres}
...
\end{equation}
such that:
\begin{equation}\label{eq:DGLAPalaSoftGlue}
...
\end{equation}
so that:
\begin{equation}\label{eq:DGLAPalaSoftGlueMuR}
...
\begin{equation}\label{eq:intrun}
