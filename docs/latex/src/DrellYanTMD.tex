...
\begin{equation}
\begin{array}{rcl}
L^{\mu\nu}(p_1,p_2) &=&\displaystyle 
\left(-g^{\mu\nu}+\frac{q^\mu
    q^\nu}{Q^2}\right)\mathcal{F}_1(z_1,z_2)+\frac{1}{p\cdot
  q}\left(p^{\mu}-\frac{p\cdot
  q}{Q^2}q^{\mu}\right)\left(p^{\nu}-\frac{p\cdot
  q}{Q^2}q^{\nu}\right)\mathcal{F}_2(z_1,z_2)\\
\\
&-&\displaystyle
    i\varepsilon^{\mu\nu\rho\sigma}\frac{q_{\rho}p_{\sigma}}{2p\cdot q}\mathcal{F}_3(z_1,z_2)\,.
\end{array}
...
\end{equation}
where $W_{\mu\nu}$ and $P_{1,2}$ are the moment of the incoming
hadrons. Notice that here we are assuming that there is non
interference between in the leptonic and the hadronic
