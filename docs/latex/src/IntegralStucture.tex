...
\label{eq:integralex}
\end{equation}
A logarithmically-spaced is defined such that
...
\begin{equation}
  w_{\alpha}^{(k)}(z) = \sum_{j=0\atop j \leq
    \alpha}^{k}\theta(z-x_{\alpha-j})\theta(x_{\alpha-j+1}-z)\prod^{k}_{\delta=0,\delta\ne
...
\begin{equation}
w_{\alpha}^{(k)}(z) = \sum_{j=0,j \leq \alpha}^{k}\theta(z-x_{\alpha-j})\theta(x_{\alpha-j+1}-z)\prod^{k}_{\delta=0,\delta\ne j}\left[\frac{1}{\delta x} \ln\left(\frac{z}{x_\alpha}\right)\frac{1}{j-\delta}+1\right].
\end{equation}
...
\label{eq:ZMconvERBL}
\end{equation}
which differs from that in Eq.~(\ref{eq:ZMconv}) in two respects: the
lower integration bound is zero rather than $x$ and the operator $O$
may also depend explicitly on the external variable $x$. Under the
assumption that this integral is convergent, we need to adapt the
strategy for the computation of numerical integrals to
...
reaches infinity but it becomes as large as $s^{\kappa}$. The size of
...
difference $\beta-\alpha$. However, this is not generally the case and
thus, at variance with the standard case, it is not possible to take
advantage of this symmetry and it becomes necessary to compute all the
$N_x^2$ integrals making up $J_{\beta\alpha}$.

\subsection{Joining grids}

As discussed above, logarithmically distributed grids are particularly
advantageous because they allow one to substantially reduce the number
of integrals to be computed. However, logarithmic grids have two main
...
discussed above while making the grid denser, and thus more accurate,
at large $x$. The implementation of this procedure in {\tt APFEL++} is
achieved by means of \textit{locked} subgrids. In practice, one starts
with a logarithmic grid with a given lower bound, \textit{e.g.}
...
possible interpolation inaccuracies at large $x$ negligible.
...
I(x_\beta) = \sum_{g=0}^{N_g-1}\int_{x_\beta/{\rm
    max}[x_\beta,x_{\gamma^{(g+1)}}]}^{x_\beta/{\rm
    max}[x_\beta,x_{\gamma^{(g)}}]}dy\,O(y)f\left(\frac{x_\beta}{y}\right)
= \sum_{g=1}^{N_g}I^{(g)}(x_\beta)\,,
\label{eq:decompDGLAP}
\end{equation}
while for and ERBL-like convolution as that in
Eq.~(\ref{eq:ZMconvERBL}) the decomposition reads:
\begin{equation}
  J(x_\beta) =
  \sum_{g=0}^{N_g-1}\int_{x_\beta/x_{\gamma^{(g+1)}}}^{x_\beta/x_{\gamma^{(g)}}}dy\,O(y,x_\beta)f\left(\frac{x_\beta}{y}\right)
  = \sum_{g=1}^{N_g}J^{(g)}(x_\beta)\,,
\label{eq:decompERBL}
\end{equation}
...
$x_\beta \geq x_{\gamma^{(g+1)}}$, since this also implies
$x_\beta \geq x_{\gamma^{(g)}}$, the integration bounds of the
integral in the $g$-th term of Eq.~(\ref{eq:decompDGLAP}) are both
equal to one giving a vanishing contribution. Therefore, as expected,
only the subgrids having $x_\beta < x_{\gamma^{(g+1)}}$ will
eventually contribute to the full integral. Conversely, all grids
contribute to the full convolution integral in
Eq.~(\ref{eq:decompERBL}), no matter the position of $x_\beta$
w.r.t. the subgrid bounds.

Let us first consider the $g$-th contribution to the integral in
Eq.~(\ref{eq:decompERBL}), $J^{(g)}$, and apply the usual
interpolation procedure to the $g$-th subgrid. To do so, we need to
...
subgrid coincide up to an index offset given by $\gamma^{(g)}$ itself
(\textit{i.e.} $x_{\alpha} =x_{\alpha-\gamma^{(g)}}^{(g)}$). This
gives:
\begin{equation}
  J^{(g)}(x_\beta) =
  \sum_{\alpha=\gamma^{(g)}}^{\gamma^{(g+1)}+\kappa^{(g)}-1}\overline{d}_\alpha
  \int_{\overline{c}}^{\overline{d}}dy\,O(y,x_\beta)w_{\alpha-\gamma^{(g)}}\left(\frac{x_{\beta}}{y}\right)=\sum_{\alpha=\gamma^{(g)}}^{\gamma^{(g+1)}+\kappa^{(g)}-1}J_{\beta\alpha}^{(g)}\,\overline{d}_\alpha\,.
\label{eq:decompERBLg}
\end{equation}
To set the upper limit of the sum ,we have exploited the fact that, if
$\alpha\geq \gamma^{(g+1)}+\kappa^{(g)}$, the lower bound of the
integral exceeds to upper one giving no contribution.  As a matter of
fact, the integration bounds are defined as:
\begin{equation}
\begin{array}{l}
...
\begin{equation}
  J_{\beta\alpha}^{(g)}=\sum_{j={\rm
      max}[0,\alpha+1-\gamma^{(g+1)}]}^{{\rm
      min}[\kappa^{(g)},\alpha-\gamma^{(g)}]}\int_{s_g^{\beta-\alpha+j-1}}^{s_g^{\beta-\alpha+j}}dy\,O(y,x_\beta)w_{\alpha-\gamma^{(g)}}\left(\frac{x_\beta}{y}\right)\,,
\label{eq:decompERBLgg}
\end{equation}
with $s_g=\exp(\delta x^{(g)})$. Eq.~(\ref{eq:decompERBL}) can finally
be written as:
\begin{equation}
  J(x_\beta) =
  \sum_{g=0}^{N_g-1}\sum_{\alpha=\gamma^{(g)}}^{\gamma^{(g+1)}+\kappa^{(g)}-1}J_{\beta\alpha}^{(g)}\,\overline{d}_\alpha
  \,.
\label{eq:decompERBL3}
\end{equation}
...
\right.\,,
\end{equation}
which allows us to exchange the sums in Eq.~(\ref{eq:decompERBL3}) and
write:
\begin{equation}
  J(x_\beta) =
...
\end{equation}
Assuming that $\overline{d}_\alpha=0$ for $\alpha\geq N_x$ and
defining:
\begin{equation}
  J_{\beta\alpha} =
...
\begin{equation}
  I^{(g)}(x_\beta) =
  \sum_{\alpha=\gamma^{(g)}}^{\gamma^{(g+1)}+\kappa^{(g)}-1}\overline{d}_\alpha
  \int_c^ddy\,O(y)w_{\alpha-\gamma^{(g)}}\left(\frac{x_{\beta}}{y}\right)=\sum_{\alpha=\gamma^{(g)}}^{\gamma^{(g+1)}+\kappa^{(g)}-1}I_{\beta\alpha}^{(g)}\,\overline{d}_\alpha\,,
\label{eq:decompDGLAPg}
\end{equation}
...
I_{\beta\alpha}^{(g)}=\sum_{j={\rm max}[0,\alpha+1-{\rm
    max}[\beta,\gamma^{(g+1)}]]}^{{\rm min}[\kappa^{(g)},\alpha-{\rm max}[\beta,\gamma^{(g)}]]}\int_{s_g^{\beta-\alpha+j-1}}^{s_g^{\beta-\alpha+j}}dy\,O(y)w_{\alpha-\gamma^{(g)}}\left(\frac{x_\beta}{y}\right)\,,
\end{equation}
allowing one to define:
\begin{equation}
  I_{\beta\alpha} =
\sum_{g=0}^{N_g-1}\theta_{\alpha,\gamma^{(g)}}\theta_{\gamma^{(g+1)}+\kappa^{(g)}-1,\alpha}I_{\beta\alpha}^{(g)}\,,
\end{equation}
such that:
\begin{equation}
  I(x_\beta) =
