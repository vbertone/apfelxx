...
\end{array}
\end{equation}
as expected. Proving that the $n=0,1$ moments diagonalise the
evolution is enough to ensure that all moments diagonalise the
evolution. The reason is that Gegenbauer polynomials obey the
following recursion relation:
\begin{equation}
  n C_{n}^{(\alpha)}(z) = 2(n+\alpha-1) zC_{n-1}^{(\alpha)}(z) -
  (n+2\alpha-2) C_{n-2}^{(\alpha)}(z)\,,\quad n=2,3,\dots\,.
\end{equation}
