...
\end{array}
\end{equation}
that automatically reduce to Eq.~(\ref{eq:MSRforward}) for
$\xi\rightarrow 0$. It is now important to verify that the splitting
functions written above obey these relations. We find:
\begin{equation}
\int_0^1dz\,z\left[\mathcal{P}_{1,\rm
    SS}\left(z,\frac{\xi}{yz}\right)+\mathcal{P}_{1,\rm
    GS}\left(z,\frac{\xi}{yz}\right)\right] = 2C_F\left[-\frac12\frac{\xi^2}{y^2-\xi^2}-\ln\left(\left|\frac{y^2}{y^2-\xi^2}\right|\right)\right]\,,
\end{equation}
and:
\begin{equation}
\int_0^{\xi/y} dz\, z\left[\mathcal{P}_{2,\rm SS}\left(z,\frac{\xi}{yz}\right)+\mathcal{P}_{2,\rm GS}\left(z,\frac{\xi}{yz}\right)\right]= 2C_F\left[\frac12\frac{\xi^2}{y^2-\xi^2}+\ln\left(\left|\frac{y^2}{y^2-\xi^2}\right|\right)\right]\,,
\end{equation}
...
\begin{equation}
\begin{array}{rcl}
\displaystyle\int_0^1dz\,z\left[\mathcal{P}_{1,\rm
    SG}\left(z,\frac{\xi}{yz}\right)+\mathcal{P}_{1,\rm
...
\begin{equation}
\begin{array}{rcl}
\displaystyle \int_0^{\xi/y} dz\, z\left[\mathcal{P}_{2,\rm
  SG}\left(z,\frac{\xi}{yz}\right)+\mathcal{P}_{2,\rm
  GG}\left(z,\frac{\xi}{yz}\right)\right]&=& \displaystyle
                                             -\frac{y^2\xi^2}{3(y^2-\xi^2)^2}\left[C_A\left(\frac{11\xi^2}{y^2}-4\right)+2n_fT_R\left(1-\frac{2\xi^2}{y^2}\right)\right]\\
\\
&+&\displaystyle 2C_A\ln\left(\left|\frac{y^2-\xi^2}{y^2}\right|\right)\,,
...
that are currently know to $\mathcal{O}(\alpha_s)$, reproduce the
well-know DIS coefficient functions in the forward limit.
...
are assuming $x_{\rm B}$ ($=x$ in our notation), we need to take
...
\begin{equation}
  ...
  \ln\left(\frac{1-y}{y}\right)-i\pi\quad\Rightarrow\quad
  \ln^2\left(-\frac{1-y}{y}-i\varepsilon\right) =
  \ln^2\left(\frac{1-y}{y}\right)-\pi^2-2\pi i \ln\left(\frac{1-y}{y}\right)\,,
\end{equation}
and:
\begin{equation}
\ln(1-\kappa-i\varepsilon) = \theta(1-\kappa) \ln(1-\kappa)+\theta(\kappa-1)\left[\ln(\kappa-1)-i\pi\right]\,.
\end{equation}
...
function. The forward limit is taken by taking $\kappa\rightarrow
0$. By doing so, we observe that the terms
$(y\rightarrow -y, \kappa\rightarrow -\kappa)$ in
Eq.~(\ref{eq:DDVCSceofffuncsNLO}) do not contribute to the imaginary
part because in the forward limit. The reason is that the integral in
...
\begin{array}{rcl}
  ...
  Im}\left[C_{q}^{(1)}(y,\kappa)\right] &=& \displaystyle C_F\left\{
...
\end{array}
\end{equation}
Now we write the logarithms in the second line as:
\begin{equation}
\ln(1-y) = -\int_0^{1}\frac{dz}{1-z}\quad\mbox{and}\quad \ln^2(1-y) =
-2 \int_0^{1}dz \frac{\ln(1-z)}{1-z}\,,
\end{equation}
...
\begin{array}{rcl}
  \displaystyle \frac{1}{2\pi}\lim_{\kappa\rightarrow 0}{\rm
  Im}\left[C_{q}^{(1)}(y,\kappa)\right] &=& \displaystyle C_F\bigg[
