...
int main()
{
  ...
  apfel::Timer t;
  const std::map<int, apfel::Distribution> pdfs = apfel::PlusMinusToPhys(EvolvedPDFs->Evaluate(mu).GetObjects());
  t.stop();

  std::cout << "Interpolation of the tabulated PDFs... ";
  t.start();
  const std::map<int, apfel::Distribution> tpdfs = apfel::PlusMinusToPhys(TabulatedPDFs.Evaluate(mu).GetObjects());
  t.stop();
  ...
  tops.SetMap(apfel::PhysicalEvolveDistributionsBasisQCD{});
  const apfel::Set<apfel::Distribution> pdfs0{apfel::PhysicalEvolveDistributionsBasisQCD{}, DistributionMap(g, apfel::LHToyPDFsPlusMinus, mu0)};
  const std::map<int, apfel::Distribution> oppdfs = apfel::PlusMinusToPhys((tops * pdfs0).GetObjects());
  ...
  for (auto const& x : xlha)
    {
      const std::map<int, double> opxpdfs = apfel::PlusMinusToPhys((apfel::Set<apfel::Distribution> {apfel::PhysicalEvolveDistributionsBasisQCD{}, tops.Evaluate(x).GetObjects()} * pdfs0).Squash());
      std::cout.precision(1);
      std::cout << x;
      std::cout.precision(4);
  ...
  for (auto const& x : xlha)
    {
      const std::map<int, double> DistMap = apfel::PlusMinusToPhys(TabulatedPDFs.EvaluateMapxQ(x, mu));
      std::cout.precision(1);
      std::cout << x;
      std::cout.precision(4);
