        ...
        for (int alpha = 0; alpha < (int) _Operator[0].size(1); alpha++)
          {
            ...
            _Operator[0](beta, alpha) += wspv * ( expr.LocalPV()
                                                  + expr.LocalLogPV(xg[beta] / xg[alpha + 1]) - expr.LocalLogPV(xg[std::max(0, alpha - k)] / xg[beta] / pow(kappa, 2)) );
          }
