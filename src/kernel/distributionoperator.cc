...
namespace apfel
{
  ...
  {
    // Stop the computation if the operators in the first term of the
    // double objects is of GPD type. Still impossible to handle here.
    ...
    for (auto const& term : DObj.GetTerms())
      {
        // Get coefficient, and distribution and operator content of
        // the current term.
  ...
  DoubleDistribution DistributionOperator::operator *= (Distribution const& d) const
  {
    ...
    if (&_grid2 != &d.GetGrid())
      throw std::runtime_error(error("DoubleDistribution::operator *=", "DistributionOperator and Distribution grids do not match."));
        ...
        for (int delta = 0; delta < nx2; delta++)
          {
            ...
            for (int alpha = m1.second; alpha < _grid1.GetSubGrid(m1.first).nx(); alpha++)
              for (int gamma = m2.second; gamma < _grid2.GetSubGrid(m2.first).nx(); gamma++)
                j(beta, delta) += _dOperator[m1.first][m2.first][jsmap1[m1.first][alpha]](0, gamma - m2.second) * dj[jsmap2[m2.first][gamma]];
          }
      }
      ...
      for (int ig2 = 0; ig2 < (int) v[ig1].size(); ig2++)
        for (int beta = 0; beta < (int) v[ig1][ig2].size(); beta++)
          for (int delta = 0; delta < (int) v[ig1][ig2][beta].size(1); delta++)
            {
              _dOperator[ig1][ig2][beta](0, delta) = 0;
              for (int gamma = 0; gamma <= delta; gamma++)
                _dOperator[ig1][ig2][beta](0, delta) += v[ig1][ig2][beta](0, gamma) * ov[ig2](0, delta - gamma);
            }
    return *this;
  }
