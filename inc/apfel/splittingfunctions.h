...
namespace apfel
{
  ...
  P2nsp(int const& nf): Expression(), _nf(nf) { }
    double Regular(double const& x) const
    {
      ...
      const auto d81    = 1. / 81.;
      const auto p2nspa =
	1641.1 - 3135. * x + 243.6 * x * x - 522.1 * pow(x,3)
	+ 128. * d81 * pow(dl,4) + 2400. * d81 * pow(dl,3)
	+ 294.9 * dl * dl + 1258. * dl
	+ 714.1 * dl1 + dl * dl1 * ( 563.9 + 256.8 * dl )
	+ _nf * ( -197.0 + 381.1 * x + 72.94 * x * x + 44.79 * pow(x,3)
                 - 192. * d81 * pow(dl,3)  - 2608. * d81 * dl * dl - 152.6 * dl
                 - 5120. * d81 * dl1 - 56.66 * dl * dl1 - 1.497 * x * pow(dl,3) )
	+ _nf * _nf * ( 32. * x * dl / ( 1. - x ) * ( 3. * dl + 10. ) + 64.
			+ ( 48. * dl * dl + 352. * dl + 384. ) * ( 1. - x ) ) * d81;
      return p2nspa;
    }
    double Singular(double const& x) const
    {
  ...
  P2nsm(int const& nf): Expression(), _nf(nf) { }
    double Regular(double const& x) const
    {
       ...
       const auto d81    = 1./81.;
       const auto p2nsma =
	 1860.2 - 3505.* x + 297.0 * x * x - 433.2 * pow(x,3)
	 + 116. * d81 * pow(dl,4) + 2880. * d81 * pow(dl,3)
	 + 399.2 * dl * dl + 1465.2 * dl
	 + 714.1 * dl1 + dl * dl1 * ( 684.0 + 251.2 * dl )
	 + _nf * ( -216.62 + 406.5 * x + 77.89 * x * x + 34.76 * pow(x,3)
		   - 256. * d81 * pow(dl,3)  - 3216. * d81 * dl * dl - 172.69 * dl
		   - 5120. * d81 * dl1 - 65.43 * dl * dl1 - 1.136 * x * pow(dl,3) )
	 + _nf * _nf * ( 32.* x * dl / (1.-x) * ( 3. * dl + 10. ) + 64.
			 + ( 48.* dl * dl + 352.* dl + 384. ) * ( 1.-x ) ) * d81;
      return p2nsma;
    }
  ...
  P2nss(int const& nf): Expression(), _nf(nf) { }
    double Regular(double const& x) const
    {
      ...
      const auto p2nssa =
	x1 * ( 151.49 + 44.51 * x - 43.12 * x * x + 4.820 * pow(x,3) )
	+ 40. * d27 * pow(dl,4) - 80. * d27 * pow(dl,3) + 6.892 * dl * dl
  ...
  P2ps(int const& nf): Expression(), _nf(nf) { }
    double Regular(double const& x) const
    {
      ...
      const auto dl1    = log(1.-x);
      const auto  p2ps1 =
	- 3584. / ( 27. * x ) * dl - 506.0 / x + 160. / 27. * pow(dl,4)
	- 400. / 9. * pow(dl,3) + 131.4 * dl * dl - 661.6 * dl
	- 5.926  * pow(dl1,3) - 9.751 * dl1 * dl1 - 72.11 * dl1
	+ 177.4 + 392.9 * x - 101.4 * x * x - 57.04 * dl * dl1;
      const auto p2ps2  =
	256. / ( 81. * x ) + 32. / 27. * pow(dl,3) + 17.89 * dl * dl
	+ 61.75 * dl + 1.778 * dl1 * dl1 + 5.944 * dl1 + 100.1
	- 125.2 * x + 49.26 * x * x - 12.59 * pow(x,3)
	- 1.889 * dl * dl1;
  ...
  P2qg(int const& nf): Expression(), _nf(nf) { }
    double Regular(double const& x) const
    {
      ...
      const auto dl1   = log(1.-x);
      const auto p2qg1 =
	- 896. / ( 3. * x ) * dl - 1268.3 / x + 536./27. * pow(dl,4)
	- 44. / 3. * pow(dl,3) + 881.5 * dl * dl + 424.9 * dl
	+ 100. / 27. * pow(dl1,4) - 70. / 9. * pow(dl1,3)
	- 120.5 * dl1 * dl1 + 104.42 * dl1
	+ 2522. - 3316. * x + 2126. * x * x
	+ dl * dl1 * ( 1823. - 25.22 * dl ) - 252.5 * x * pow(dl,3);
      const auto p2qg2 =
	1112. / ( 243. * x ) - 16. / 9. * pow(dl,4)
	- 376. / 27. * pow(dl,3) - 90.8 * dl * dl - 254.0 * dl
	+ 20./27. * pow(dl1,3) + 200. / 27. * dl1 * dl1 - 5.496 * dl1
	- 252.0  + 158.0 * x + 145.4 * x * x - 139.28 * pow(x,3)
	- dl * dl1 * ( 53.09  + 80.616 * dl ) - 98.07 * x * dl * dl
	+ 11.70 * x * pow(dl,3);
  ...
  P2gq(int const& nf): Expression(), _nf(nf) { }
    double Regular(double const& x) const
    {
      ...
      const auto dl1 = log(1.-x);
      const auto p2gq0 =
	1189.3 * dl / x + 6163.1 / x - 4288. / 81. * pow(dl,4)
	+ 1568. / 9. * pow(dl,3) - 1794. * dl * dl + 4033. * dl
	+ 400. / 81. * pow(dl1,4) + 2200. / 27. * pow(dl1,3)
	+ 606.3 * dl1 * dl1 + 2193. * dl1
	- 4307. + 489.3 * x + 1452.* x * x + 146.0 * pow(x,3)
	- 447.3 * dl * dl * dl1 - 972.9 * x * dl * dl;
     const auto p2gq1 =
       71.082 * dl / x  - 46.41 / x + 128. / 27. * pow(dl,4)
       + 704/81. * pow(dl,3) + 20.39 * dl * dl + 174.8 * dl
       - 400./81. * pow(dl1,3) - 68.069 * dl1 * dl1 - 296.7 * dl1
       - 183.8 + 33.35 * x - 277.9 * x * x + 108.6 * x * dl * dl
       - 49.68 * dl * dl1;
     const auto p2gq2 =
       ( 64. * ( - 1. / x + 1. + 2. * x )
	 + 320. * dl1 * ( 1. / x - 1. + 0.8 * x )
	 + 96. * dl1 * dl1 * ( 1. / x - 1. + 0.5 * x ) ) / 27.;
  ...
  P2gg(int const& nf): Expression(), _nf(nf) { }
    double Regular(double const& x) const
    {
      ...
      const auto dl1    = log(1.-x);
      const auto p2gga0 =
	2675.8 * dl / x + 14214. / x - 144. * pow(dl,4) + 72. * pow(dl,3)
	- 7471. * dl * dl + 274.4 * dl + 3589. * dl1 - 20852.
	+ 3968. * x - 3363. * x * x + 4848. * pow(x,3)
	+ dl * dl1 * ( 7305. + 8757. * dl );
      const auto p2gga1 =
	157.27 * dl / x + 182.96 / x + 512./27. * pow(dl,4)
	+ 832. / 9. * pow(dl,3) + 491.3 * dl * dl + 1541. * dl
	- 320.0 * dl1 - 350.2 + 755.7 * x - 713.8 * x * x
	+ 559.3 * pow(x,3) + dl * dl1 * ( 26.15 - 808.7 * dl );
     const auto p2gga2 =
       - 680. / ( 243. * x ) - 32. / 27. * pow(dl,3) + 9.680 * dl * dl
       - 3.422 * dl - 13.878 + 153.4 * x - 187.7 * x * x
       + 52.75 * pow(x,3) - dl * dl1 * ( 115.6 - 85.25 * x + 63.23 * dl);
